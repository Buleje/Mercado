...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await withCronRetry("price-drop-alerts", async () => {
      ...
      // 1. Find products with price drops in the last 24h
      const priceDrops = await prisma.priceHistory.findMany({
        where: {
          changedAt: { gte: oneDayAgo },
          // Only actual drops (newPrice < oldPrice)
        },
        select: {
          productId: true,
          oldPrice: true,
      ...
      if (dropsByProduct.size === 0) {
        return { ok: true, notifications: 0, priceDrops: 0, reason: "Sin bajas de precio hoy" };
      }

      const droppedProductIds = Array.from(dropsByProduct.keys());

      // 2. Find ShoppingListItems referencing these products
      ...
      if (listItems.length === 0) {
        return {
          ok: true,
          notifications: 0,
          priceDrops: dropsByProduct.size,
          reason: "Nadie tiene estos productos en su lista",
        };
      }

      // 3. Get product names for the message
      const products = await prisma.product.findMany({
        where: { id: { in: droppedProductIds } },
      ...
      // 5. Send notifications (fire-and-forget, max 50 per run)
      let sent = 0;
      ...
      logActivity(
        "price-drop-alerts",
        "Notification",
        `${sent} clientes notificados sobre ${dropsByProduct.size} productos con baja de precio`,
        undefined,
        "cron"
      ).catch(() => {});

      logger.info("[cron/price-drop-alerts] Notificaciones enviadas", {
        priceDrops: dropsByProduct.size,
        notifications: sent,
      });
