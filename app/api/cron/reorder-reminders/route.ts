...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await withCronRetry("reorder-reminders", async () => {
      ...
      // Fetch delivered orders from last 90 days with items and customer phone
      ...
      if (orders.length === 0) {
        logger.info("[cron/reorder-reminders] Sin pedidos recientes para analizar");
        return { ok: true, reminders: 0, analyzed: 0 };
      }
      ...
      for (const [phone, data] of customerOrders) {
        ...
        // Max 50 reminders per cron run to avoid overwhelming
        if (remindersSent >= 50) break;
      }

      logger.info("[cron/reorder-reminders] Recordatorios enviados", {
        analyzed: customerOrders.size,
        remindersSent,
      });

      return { ok: true, reminders: remindersSent, analyzed: customerOrders.size };
    });
