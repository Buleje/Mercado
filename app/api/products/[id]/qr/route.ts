...
function qrToSVG(matrix: boolean[][], productName: string, productPrice: number): string {
  ...
  rects.push(
    `<text x="${totalW / 2}" y="${labelY + 40}" font-family="Arial,sans-serif" font-size="14" font-weight="bold" fill="var(--accent)" text-anchor="middle">S/ ${productPrice.toFixed(2)}</text>`
  );
