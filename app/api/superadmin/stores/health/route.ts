...
export async function GET(req: NextRequest) {
  try {
    ...
    // [PERF] Cache 10min: escaneaba todos los tenants 'store' + settings + product
    // counts y armaba ~17 checks por tienda en CADA carga del dashboard (no-store).
    const payload = await getOrSet("superadmin:stores-health", 600, async () => {
      ...
      const items = tenants.map((t) => {
        ...
        const productCount = (productCountMap.get(t.id) ?? 0) + (productCountMap.get(t.slug) ?? 0);
        ...
        checks.push({
          ...
          status: store?.isPublished ? "done" : store ? "missing" : "missing",
          help: store
