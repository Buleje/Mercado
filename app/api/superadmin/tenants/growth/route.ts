...
export async function GET(req: NextRequest) {
  try {
    ...
    // [PERF] Cache 10min. NO se reescribió a date_trunc('month'): la verificación
    // before/after (script con Prisma real) probó que date_trunc desplaza meses en
    // el borde por timezone (17/17 celdas, shift de 1 mes) y corrompería el revenue
    // mensual. Cachear preserva el cómputo EXACTO (rebucket JS por monthKey).
    const data = await getOrSet("superadmin:tenants-growth", 600, async () => {
      const now = new Date();
      ...
      return Object.values(tenantGrowth).sort((a, b) => b.totalRevenue - a.totalRevenue);
    });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error("[superadmin/tenants/growth] Error fetching growth data", {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
    return NextResponse.json({ error: "Error loading growth data", data: [] }, { status: 500 });
  }
}
