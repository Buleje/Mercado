...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const categories: DemandInput["categories"] = Array.from(
      allCategories,
    ).map((cat) => ({
      category: cat,
    ...
    const stockouts: DemandInput["stockouts"] = Array.from(
      stockoutMap.values(),
    ).map((s) => ({
      name: s.name,
      category: s.category,
    ...
    const zoneAcc = new Map<
      string,
      { units: number; categoryUnits: Map<string, number> }
    >();
    ...
    for (const order of ordersForZones) {
      ...
      for (const item of order.items) {
        ...
        if (cat) {
          acc.categoryUnits.set(
            cat,
            (acc.categoryUnits.get(cat) ?? 0) + item.quantity,
          );
        }
      }
    ...
    for (const s of rawStockouts) {
      tenantStockoutCount.set(
        s.tenantId,
        (tenantStockoutCount.get(s.tenantId) ?? 0) + 1,
      );
    }
    ...
    const zones: DemandInput["zones"] = Array.from(zoneAcc.entries()).map(
      ([zone, acc]) => ({
        zone,
        units: acc.units,
        categoryUnits: Array.from(acc.categoryUnits.entries()).map(
    ...
    const report = buildDemandReport(input, now, PERIOD_DAYS, PERIOD_LABEL);
