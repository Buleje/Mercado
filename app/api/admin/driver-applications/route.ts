...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json().catch((err) => { logger.error("[admin/driver-applications] parse JSON body failed", { error: String(err) }); return null; });
  ...
  try {
    ...
    if (parsed.data.action === "approve") {
      ...
      const partner = await prisma.deliveryPartner.create({
        ...
        select: { id: true },
      }).catch((err) => { logger.error("[admin/driver-applications] operation failed", { error: String(err), tenantId: auth.tenantId }); return null; }); // Don't fail if duplicate
      ...
      if (phone) {
        ...
        await sendWhatsAppQueued(phone, message, { tenantId: auth.tenantId, context: "driver-applications:approve" }).catch((err) => logger.error("[admin/driver-applications] WhatsApp enqueue fallback failed", { error: String(err), tenantId: auth.tenantId }));
      }
    } else {
      if (phone) {
        ...
        await sendWhatsAppQueued(phone, message, { tenantId: auth.tenantId, context: "driver-applications:reject" }).catch((err) => logger.error("[admin/driver-applications] WhatsApp enqueue fallback failed", { error: String(err), tenantId: auth.tenantId }));
      }
    }
