...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch((err) => {
    logger.warn("documents.templates.generate.body_parse_fail", { err: String(err) });
    return {};
  });
  ...
  try {
    ...
    const tpl = await DocumentsDB.getTemplate(auth.tenantId, parsed.data.templateKey);
    ...
    }).catch((err) => logger.warn("documents.audit.template_fail", { err: String(err) }));
