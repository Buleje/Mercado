...
const SECTIONS: { id: SectionId; label: string; icon: typeof Sparkles; description: string }[] = [
  {
    ...
    label: "Identidad",
    icon: Sparkles,
    description: "Nombre, tagline y descripción de la plataforma.",
  },
  {
    id: "logos",
    label: "Logos",
    icon: ImageIcon,
    description: "Logo claro, oscuro, cuadrado, favicon y OG.",
  },
  {
    id: "colors",
    label: "Colores",
    icon: Palette,
    description: "Paleta de marca: primary, secondary, accent, alertas.",
  },
  {
    id: "typography",
    label: "Tipografía",
    icon: Type,
    description: "Familia para body y para títulos display.",
  },
  {
    id: "contact",
    label: "Contacto",
    icon: Phone,
    description: "Email, teléfono, WhatsApp, dirección, horarios.",
  },
  {
    id: "socials",
    label: "Redes sociales",
    icon: Hash,
    description: "Links a Instagram, Facebook, TikTok, X, YouTube.",
  },
  {
    id: "seo",
    label: "SEO & Open Graph",
    icon: Search,
    description: "Meta title/description y previsualización social.",
  },
  {
    id: "legal",
    label: "Legal",
    icon: Scale,
    description: "Razón social, RUC, términos, privacidad.",
  },
  {
    id: "event",
    label: "Modo evento",
    icon: CalendarClock,
    description: "Override temporal de logo + colores (Black Friday, Navidad).",
  },
];
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-stretch gap-2 flex-wrap">
              ...
              {brand.eventMode.active && (
                <StatPill
                  label={`Evento: ${brand.eventMode.name || "activo"}`}
                  value="ON"
                  accent="warning"
                />
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
          ...
          <main className="space-y-4">
            {activeSection === "identity" && (
              <IdentitySection brand={brand} onPatch={(p) => patch("identity", p)} />
            )}
            {activeSection === "logos" && (
              <LogosSection brand={brand} onPatch={(p) => patch("logos", p)} />
            )}
            {activeSection === "colors" && (
              <ColorsSection brand={brand} onPatch={(p) => patch("colors", p)} />
            )}
            {activeSection === "typography" && (
              <TypographySection brand={brand} onPatch={(p) => patch("typography", p)} />
            )}
            {activeSection === "contact" && (
              <ContactSection brand={brand} onPatch={(p) => patch("contact", p)} />
            )}
            {activeSection === "socials" && (
              <SocialsSection brand={brand} onPatch={(p) => patch("socials", p)} />
            )}
            {activeSection === "seo" && (
              <SeoSection brand={brand} onPatch={(p) => patch("seo", p)} />
            )}
            {activeSection === "legal" && (
              <LegalSection brand={brand} onPatch={(p) => patch("legal", p)} />
            )}
            {activeSection === "event" && (
              <EventSection brand={brand} onPatch={(p) => patch("eventMode", p)} />
            )}
...
function IdentitySection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["identity"]>) => void;
}) {
  return (
    <SectionShell
      title="Identidad"
      description="Cómo se llama y qué hace tu plataforma. Aparece en header, footer, SEO y compartidos sociales."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Nombre de la plataforma" hint="2-30 caracteres · aparece en navbar y footer">
          <TextInput
            value={brand.identity.name}
        ...
        <div className="sm:col-span-2">
          <Field
            label="Descripción"
            hint="Aparece en el footer y como descripción larga en redes sociales"
          >
            <TextArea
              value={brand.identity.description}
...
function LogosSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["logos"]>) => void;
}) {
  const SLOTS: Array<{
    key: keyof PlatformBrand["logos"];
    label: string;
    hint: string;
    aspect: string;
    mode: "wide" | "square";
  }> = [
    {
      ...
      hint: "Para fondos claros · navbar/footer · 240×60 (4:1)",
      aspect: "aspect-[4/1]",
      mode: "wide",
    },
    {
      key: "logoDark",
      label: "Logo oscuro",
      hint: "Para fondos oscuros · header sticky/dark mode · 240×60",
      aspect: "aspect-[4/1]",
      mode: "wide",
    },
    {
      key: "logoSquare",
      label: "Logo cuadrado",
      hint: "App icon · 512×512 PNG transparente",
    ...
    },
    {
      key: "favicon",
      label: "Favicon",
      hint: "Icono de pestaña · 32×32 ICO/PNG",
      aspect: "aspect-square",
      mode: "square",
    },
    {
      ...
      hint: "Para compartir en WhatsApp/FB · 1200×630",
...
function ColorsSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["colors"]>) => void;
}) {
  const SLOTS: Array<{ key: keyof PlatformBrand["colors"]; label: string; hint: string }> = [
    {
      key: "primary",
      label: "Primary",
      hint: "Acento de marca · botones principales, links activos",
    },
  ...
  return (
    <SectionShell
      title="Colores"
      description="Paleta de marca. Cada color se aplica vía CSS variables en todas las páginas."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {SLOTS.map((s) => (
          <div
...
function TypographySection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["typography"]>) => void;
}) {
  ...
  return (
    <SectionShell
      title="Tipografía"
      description="Familia para body (UI normal) y display (títulos editoriales)."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
function ContactSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["contact"]>) => void;
}) {
  return (
    <SectionShell
      title="Contacto"
      description="Datos que aparecen en footer, página de soporte y meta-información."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Horarios de soporte">
          <TextInput
            value={brand.contact.supportHours}
            onChange={(e) => onPatch({ supportHours: e.target.value })}
        ...
        <div className="sm:col-span-2">
          <Field label="Dirección">
            <TextInput
              ...
              placeholder="Pucallpa, Ucayali — Perú"
            />
          </Field>
        </div>
      </div>
...
function SocialsSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["socials"]>) => void;
}) {
...
function SeoSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["seo"]>) => void;
}) {
  return (
    <SectionShell
      title="SEO & Open Graph"
      description="Cómo aparece tu sitio en Google y al compartir en WhatsApp/Facebook."
    >
      <div className="grid grid-cols-1 gap-4">
        <Field label="Meta title" hint="Máximo 70 caracteres · aparece en Google">
          <TextInput
            value={brand.seo.metaTitle}
            onChange={(e) => onPatch({ metaTitle: e.target.value })}
            maxLength={70}
          />
          <p className="text-[length:var(--ts-2xs)] text-[var(--text-tertiary)] mt-1 tabular-nums text-right">
            {brand.seo.metaTitle.length}/70
          </p>
        </Field>
        <Field
          label="Meta description"
          hint="Máximo 180 caracteres · aparece bajo el título en Google"
        >
          <TextArea
          ...
          />
          <p className="text-[length:var(--ts-2xs)] text-[var(--text-tertiary)] mt-1 tabular-nums text-right">
            {brand.seo.metaDescription.length}/180
          </p>
        </Field>
        <Field label="Open Graph title" hint="Cuando comparten link en WhatsApp/Facebook">
          <TextInput
            value={brand.seo.ogTitle}
            onChange={(e) => onPatch({ ogTitle: e.target.value })}
            maxLength={80}
          />
        </Field>
        <Field label="Open Graph description">
          <TextArea
            value={brand.seo.ogDescription}
            onChange={(e) => onPatch({ ogDescription: e.target.value })}
            maxLength={200}
          />
        </Field>
      </div>
      ...
      <div className="mt-5 rounded-xl border border-dashed border-[var(--rule-base)] bg-[var(--surface-canvas)] p-5">
        ...
        <p className="text-lg text-blue-700 font-medium mt-0.5 hover:underline cursor-pointer">
          {brand.seo.metaTitle || "(sin título)"}
        </p>
        <p className="text-sm text-[var(--text-secondary)] mt-1 leading-snug">
          {brand.seo.metaDescription || "(sin descripción)"}
        </p>
      </div>
    </SectionShell>
  );
...
function LegalSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["legal"]>) => void;
}) {
  return (
    <SectionShell
      title="Legal"
      description="Datos legales que aparecen en footer y en facturación electrónica (SUNAT)."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Razón social" hint="Nombre legal completo (con S.A.C., E.I.R.L., etc.)">
          <TextInput
            value={brand.legal.legalName}
            onChange={(e) => onPatch({ legalName: e.target.value })}
          />
        </Field>
        <Field label="RUC" hint="11 dígitos">
          <TextInput
            value={brand.legal.ruc}
            onChange={(e) => onPatch({ ruc: e.target.value })}
            maxLength={11}
            className="font-mono"
          />
        </Field>
        <Field label="URL de Términos">
          <TextInput
            value={brand.legal.termsUrl}
            onChange={(e) => onPatch({ termsUrl: e.target.value })}
            placeholder="/terminos"
          />
        </Field>
        <Field label="URL de Privacidad">
          <TextInput
            value={brand.legal.privacyUrl}
            onChange={(e) => onPatch({ privacyUrl: e.target.value })}
            placeholder="/privacidad"
          />
        </Field>
        <Field label="URL de Cookies">
          <TextInput
            value={brand.legal.cookiesUrl}
            onChange={(e) => onPatch({ cookiesUrl: e.target.value })}
            placeholder="/cookies"
          />
        </Field>
      </div>
...
function EventSection({
  ...
  brand: PlatformBrand;
  onPatch: (p: Partial<PlatformBrand["eventMode"]>) => void;
}) {
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Fecha de inicio" hint="Opcional · si vacía, aplica desde 'activo'">
          <TextInput
            type="datetime-local"
            value={e.startsAt ? e.startsAt.slice(0, 16) : ""}
            onChange={(ev) => onPatch({ startsAt: ev.target.value || null })}
          />
        </Field>
        <Field label="Fecha de fin" hint="Opcional · si vacía, aplica indefinidamente">
          <TextInput
            type="datetime-local"
            value={e.endsAt ? e.endsAt.slice(0, 16) : ""}
            onChange={(ev) => onPatch({ endsAt: ev.target.value || null })}
          />
        </Field>
