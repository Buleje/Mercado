...
function SourceBadge({ status }: { status: SloSourceStatus }) {
  const cfg: Record<SloSourceStatus, { label: string; cls: string }> = {
    ...
    error: {
      ...
      cls: "bg-[var(--data-error-100)] text-[var(--data-error-500)]",
    },
  };
  const { label, cls } = cfg[status];
  return (
    <span className={`text-xs font-semibold px-2 py-0.5 rounded ml-auto ${cls}`}>{label}</span>
  );
}
...
function SourceEmptyState({ status, envHint }: { status: SloSourceStatus; envHint: string }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2">
      <Cable
        className={`w-5 h-5 shrink-0 mt-0.5 ${isError ? "text-[var(--data-error-500)]" : "text-[var(--text-tertiary)]"}`}
      />
  ...
  return (
    <div className={`rounded-xl border-2 p-5 ${colors[status]} ${bg[status]}`}>
      ...
      <p className="text-3xl font-extrabold tabular-nums text-[var(--text-primary)]">
        {value}
        {unit && (
          <span className="text-base font-medium ml-1 text-[var(--text-tertiary)]">{unit}</span>
        )}
      </p>
    ...
    <>
      ...
      <AdminTabShell
        info={{
          ...
          affects:
            "Solo lectura para el superadmin. No modifica ningún dato de las tiendas. Las alertas aquí reflejan el estado real de la infraestructura compartida.",
          example:
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <SloCard
            ...
            status={
              !deployLive
                ? "warning"
                : data.deploy!.state === "READY"
                  ? "ok"
                  : data.deploy!.state === "ERROR"
                    ? "error"
                    : "warning"
            }
            ...
            <div className="space-y-3">
              {data.sentryErrors.map((e) => (
                <div key={e.type} className="flex items-center gap-4">
                  <span className="text-sm font-mono text-[var(--text-secondary)] flex-1 truncate">
                    {e.type}
                  </span>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold tabular-nums w-8 text-right text-[var(--text-primary)]">
                      {e.count}
                    </span>
                ...
                {data.checkoutFunnel.map((step, i) => {
                  ...
                  return (
                    <div key={step.step} className="flex items-center gap-3">
                      <span className="text-xs text-[var(--text-tertiary)] w-4 tabular-nums">
                        {i + 1}.
                      </span>
                      <span className="text-sm text-[var(--text-secondary)] w-36 truncate">
                        {step.step}
                      </span>
        ...
        {data.cronHealth.length > 0 && (
          <div className="bg-[var(--surface-canvas)] border border-[var(--rule-base)] rounded-xl p-6">
            <h3 className="text-base font-bold text-[var(--text-primary)] flex items-center gap-2 mb-4">
              ...
              <span className="text-xs bg-[var(--data-success-100)] text-[var(--data-success-500)] px-2 py-0.5 rounded ml-auto">
                datos reales
              </span>
            </h3>
            <div className="space-y-2">
              {data.cronHealth.slice(0, 15).map((cron) => (
                <div key={cron.jobName} className="flex items-center gap-3">
                  ...
                  <span className="text-xs tabular-nums text-[var(--text-tertiary)] w-16 text-right">
