...
interface TenantSettings {
  ...
  storeTheme?: { primaryColor?: string };
  logoText?: string;
  currency?: string;
}
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col">
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        <div className="flex-1">
          ...
          <p className="font-semibold text-gray-900 dark:text-white text-sm leading-tight line-clamp-2 mt-0.5">
            {product.name}
          </p>
        </div>
        ...
        {qty === 0 ? (
          ...
          <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden">
            <button
              onClick={onRemove}
...
export default function TenantStorefront() {
  ...
  const primaryColor = settings.storeTheme?.primaryColor ?? "#00B4A6";
  ...
  if (loadingSettings || loadingProducts) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-950">
        <Loader2 className="w-8 h-8 animate-spin" style={{ color: "#00B4A6" }} />
        <p className="text-sm text-gray-500 dark:text-gray-400">Cargando tienda…</p>
      </div>
    );
  }
  ...
  if (!tenantExists) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-950 p-4 text-center">
        <PackageX className="w-14 h-14 text-gray-300 dark:text-gray-700" />
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Tienda no disponible</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm">
          La tienda &quot;{slug}&quot; no existe o no está activa.
        </p>
      </div>
    );
  }
        ...
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          {cartCount > 0 && (
            ...
            >
              ...
              <span
                className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full text-xs font-bold flex items-center justify-center bg-[#f4a261] text-white"
              >
