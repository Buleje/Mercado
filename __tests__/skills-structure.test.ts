  ...
  skillFiles.forEach((file) => {
    describe(`Skill: ${file}`, () => {
      ...
      it('should contain a "Contexto" section', () => {
        const hasContexto =
          /^## Contexto del proyecto/m.test(content) || /^## Contexto/m.test(content);
        expect(hasContexto).toBe(true);
      });

      it('should contain a "Reglas" section', () => {
        const hasReglas = /^## Reglas fundamentales/m.test(content) || /^## Reglas/m.test(content);
        expect(hasReglas).toBe(true);
      });

      it('should contain a "Patrones" section', () => {
        const hasPatrones =
          /^## Patrones aprobados/m.test(content) || /^## Patrones/m.test(content);
  ...
  agentFiles.forEach((file) => {
    describe(`Agent: ${file}`, () => {
      ...
      it('should contain a "Rol" section', () => {
        const hasRol =
          /^## Rol/m.test(content) || /^# Rol/m.test(content) || /^# Role/m.test(content);
        expect(hasRol).toBe(true);
      });

      it('should contain a "Responsabilidades" section', () => {
        const hasResponsabilidades =
          /^## Responsabilidades/m.test(content) || /^# Mission/m.test(content);
        expect(hasResponsabilidades).toBe(true);
      });

      it('should contain a "Skills vinculados" section', () => {
        const hasSkills =
          /^## Skills vinculados/m.test(content) || /^# Skills vinculados/m.test(content);
