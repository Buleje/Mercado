...
describe.skipIf(!HAS_DB)("LO-TH · invariantes de cadena de custodia (ADR-305)", () => {
  it("T3 — no se tala dos veces el mismo árbol", async () => {
    const tree = `${P}-A1`;
    await crear({ section: "tala", treeCode: tree, speciesCommon: "Tornillo", volumeM3: 8 });
    await expect(crear({ section: "tala", treeCode: tree, speciesCommon: "Tornillo", volumeM3: 8 }))
      .rejects.toMatchObject({ name: "LothInvariantError", code: "T3_TALA_DUPLICADA" });
  });

  it("T3 — trozaCode único en Trozado", async () => {
    const troza = `${P}-B1-A`;
    await crear({ section: "trozado", treeCode: `${P}-B1`, trozaCode: troza, speciesCommon: "Tornillo", volumeM3: 1 });
    await expect(crear({ section: "trozado", treeCode: `${P}-B1`, trozaCode: troza, speciesCommon: "Tornillo", volumeM3: 1 }))
      .rejects.toMatchObject({ code: "T3_TROZA_DUPLICADA" });
  });

  it("T4 — no se troza más de lo tumbado", async () => {
    const tree = `${P}-C1`;
    await crear({ section: "tala", treeCode: tree, speciesCommon: "Tornillo", volumeM3: 10 });
    await crear({ section: "trozado", treeCode: tree, trozaCode: `${tree}-A`, speciesCommon: "Tornillo", volumeM3: 6 });
    // 6 + 5 = 11 > 10 tumbado → rechazo
    await expect(crear({ section: "trozado", treeCode: tree, trozaCode: `${tree}-B`, speciesCommon: "Tornillo", volumeM3: 5 }))
      .rejects.toMatchObject({ code: "T4_TROZADO_SUPERA_TALA" });
    // 6 + 4 = 10 ≤ 10 → pasa (merma cero es válida)
    await expect(crear({ section: "trozado", treeCode: tree, trozaCode: `${tree}-C`, speciesCommon: "Tornillo", volumeM3: 4 }))
      .resolves.toBeTruthy();
  });
  ...
  it("T1 — una troza sale del bosque una sola vez (despacho, luego consumo)", async () => {
    const tree = `${P}-D1`;
    const troza = `${tree}-A`;
    await crear({ section: "tala", treeCode: tree, speciesCommon: "Tornillo", volumeM3: 5 });
    await crear({ section: "trozado", treeCode: tree, trozaCode: troza, speciesCommon: "Tornillo", volumeM3: 4 });
    ...
    await expect(crear({ section: "consumo_troza", trozaCode: troza, speciesCommon: "Tornillo", volumeM3: 4 }))
      .rejects.toMatchObject({ code: "T1_TROZA_YA_MOVILIZADA" });
  });

  it("T5 — no se despacha más producto del producido", async () => {
    const prod = `${P}-PT1`;
    await crear({ section: "producto_terminado", productType: prod, speciesCommon: "Tornillo", quantity: 10, unit: "m3" });
    await crear({ section: "despacho_producto", productType: prod, speciesCommon: "Tornillo", quantity: 6, unit: "m3", gtfNumber: `${P}-PT1G` });
    // 6 + 5 = 11 > 10 → rechazo
    await expect(crear({ section: "despacho_producto", productType: prod, speciesCommon: "Tornillo", quantity: 5, unit: "m3", gtfNumber: `${P}-PT1G2` }))
      .rejects.toMatchObject({ code: "T5_DESPACHO_SUPERA_PRODUCCION" });
    // 6 + 4 = 10 ≤ 10 → pasa
    await expect(crear({ section: "despacho_producto", productType: prod, speciesCommon: "Tornillo", quantity: 4, unit: "m3", gtfNumber: `${P}-PT1G3` }))
      .resolves.toBeTruthy();
  });
  ...
  it("cadena feliz completa: tala → trozado → despacho pasa sin trabas", async () => {
    const tree = `${P}-E1`;
    const troza = `${tree}-A`;
    await crear({ section: "tala", treeCode: tree, speciesCommon: "Capirona", volumeM3: 6 });
    await crear({ section: "trozado", treeCode: tree, trozaCode: troza, speciesCommon: "Capirona", volumeM3: 5 });
