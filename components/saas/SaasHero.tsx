          ...
          {CHART_BARS.map((bar) => (
            <div key={bar.label} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full relative rounded-t" style={{ height: `${bar.h}%` }}>
                <div className="w-full h-full rounded-t" style={{ background: bar.today ? "linear-gradient(to top, #0f766e, #2dd4bf)" : "rgba(15,118,110,0.35)" }} />
          ...
          <AnimatePresence mode="wait">
            {ventas.map((v, i) => (
              <motion.div key={`${vcy}-${i}`} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 8 }} transition={{ duration: 0.25, delay: i * 0.06 }} className="flex items-center justify-between text-xs rounded-lg px-2.5 py-1.5" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <span style={{ color: "#2dd4bf" }} className="font-mono font-semibold">{v.m}</span>
    ...
    <div className="p-3 flex gap-3" style={{ minHeight: "220px" }}>
      <div className="flex-1">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {PRODUCTOS.map((p) => (
            <button key={p.nombre} type="button" onClick={() => agregar(p)} className="text-left rounded-xl p-2 border transition-all duration-150 hover:scale-[1.03] active:scale-[0.97] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0f766e]" style={{ background: "rgba(15,118,110,0.08)", borderColor: "rgba(15,118,110,0.2)", minHeight: "44px" }}>
      ...
      <div className="w-28 flex flex-col gap-2">
        ...
        {carrito.length > 0 && (
          <div>
            <p className="text-xs text-gray-400 mb-1">Total: <span className="font-mono font-bold" style={{ color: "#2dd4bf" }}>S/ {total.toFixed(2)}</span></p>
            <AnimatePresence mode="wait">
              {cobrado
                ...
                : <motion.button key="cobrar" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} type="button" onClick={cobrar} className="w-full text-xs font-bold py-1.5 rounded-xl text-white active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0f766e]" style={{ background: "linear-gradient(135deg, #0f766e, #0d5f58)", minHeight: "32px" }}>Cobrar S/ {total.toFixed(2)}</motion.button>
              }
            </AnimatePresence>
          </div>
        )}
  ...
  return (
    <div className="p-3" style={{ minHeight: "220px" }}>
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl mb-3" style={{ background: "rgba(15,118,110,0.15)", border: "1px solid rgba(15,118,110,0.25)" }}>
        <div className="h-7 w-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: "#0f766e" }} aria-hidden="true">WA</div>
      ...
      <div className="space-y-2 overflow-hidden">
        <AnimatePresence>
          {CHAT.slice(0, vis).map((msg, i) => (
            <motion.div key={i} variants={bubbleV} initial="initial" animate="animate" exit="exit" className={`flex ${msg.tipo === "bot" ? "justify-start" : "justify-end"}`}>
              <div className="max-w-[85%] text-xs px-2.5 py-1.5 rounded-2xl leading-relaxed" style={msg.tipo === "bot" ? { background: "rgba(15,118,110,0.25)", color: "#e2e8f0", borderBottomLeftRadius: "4px" } : { background: "rgba(255,255,255,0.1)", color: "#d1d5db", borderBottomRightRadius: "4px" }}>
                {msg.texto}
              </div>
            </motion.div>
          ))}
        ...
        {vis > 0 && vis < CHAT.length && CHAT[vis]?.tipo === "bot" && (
          <div className="flex justify-start">
            <div className="flex items-center gap-1 px-3 py-2 rounded-2xl" style={{ background: "rgba(15,118,110,0.2)", borderBottomLeftRadius: "4px" }} aria-label="Escribiendo...">
              {[0, 1, 2].map((d) => <span key={d} className="h-1.5 w-1.5 rounded-full animate-bounce" style={{ background: "#2dd4bf", animationDelay: `${d * 150}ms` }} aria-hidden="true" />)}
            </div>
          </div>
        )}
      </div>
...
const StatCounter = memo(function StatCounter({ target, suffix, label, delay }: { target: number; suffix: string; label: string; delay: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay, duration: 0.5, ease: SPRING }} className="text-center">
      <p className="text-4xl sm:text-5xl font-black tracking-tight font-mono" style={{ color: "#0f766e", letterSpacing: "-0.03em" }}>{count}{suffix}</p>
      <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 mt-1 font-medium">{label}</p>
    </motion.div>
  );
});

const DotBg = <div className="absolute inset-0 pointer-events-none" aria-hidden="true" style={{ backgroundImage: "radial-gradient(circle, #0f766e10 1px, transparent 1px)", backgroundSize: "24px 24px" }} />;
const GlowBg = <div className="absolute inset-0 pointer-events-none" aria-hidden="true" style={{ background: "radial-gradient(ellipse 70% 50% at 50% -10%, rgba(15,118,110,0.12) 0%, transparent 70%)" }} />;
  ...
  return (
    <section className="relative pt-24 pb-16 sm:pt-32 sm:pb-24 overflow-hidden" aria-label="Seccion principal" onMouseMove={onMouseMove} onMouseLeave={onMouseLeave}>
      ...
      <div ref={glowRef} className="absolute pointer-events-none opacity-0 transition-opacity duration-300 hidden lg:block" aria-hidden="true"
        style={{ width: "400px", height: "400px", top: 0, left: 0, background: "radial-gradient(circle, rgba(15,118,110,0.15) 0%, transparent 70%)", borderRadius: "50%", willChange: "transform" }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.div variants={stagger} initial="initial" animate="animate" className="flex flex-col items-start">
            <motion.div variants={reduced ? {} : fadeUp} className="mb-5 inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold text-white shadow-lg"
              style={{ background: "linear-gradient(135deg, #0f766e 0%, #0d5f58 100%)", boxShadow: "0 4px 20px -4px rgba(15,118,110,0.5)" }}>
            ...
            <motion.h1 variants={reduced ? {} : fadeUp} className="text-4xl sm:text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white leading-tight mb-6" style={{ letterSpacing: "-0.03em" }}>
              Tu bodega merece un sistema{" "}<span style={{ color: "#0f766e" }}>inteligente</span>
            </motion.h1>
          ...
          <motion.div initial={reduced ? {} : { opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2, ease: SPRING }} className="relative w-full">
            <div className="absolute -inset-8 rounded-3xl pointer-events-none" aria-hidden="true" style={{ boxShadow: "0 0 120px 40px rgba(15,118,110,0.15)" }} />
            <div className="absolute -inset-8 rounded-3xl pointer-events-none" aria-hidden="true" style={{ background: "radial-gradient(ellipse at 50% 50%, rgba(15,118,110,0.12) 0%, transparent 70%)", filter: "blur(24px)" }} />

            <div ref={mockupRef} className="relative rounded-2xl overflow-hidden border shadow-2xl"
              style={{ background: "#111827", borderColor: "rgba(15,118,110,0.35)", boxShadow: "0 32px 64px -16px rgba(0,0,0,0.6), 0 0 0 1px rgba(15,118,110,0.15)", transition: "transform 0.15s ease-out", willChange: "transform" }}>
              ...
              <div className="flex items-center gap-1 px-3 py-2 border-b" style={{ background: "#1a2535", borderColor: "rgba(255,255,255,0.06)" }} role="tablist" aria-label="Modulos del sistema">
                {TABS.map((t) => (
                  <button key={t} type="button" role="tab" aria-selected={tab === t} onClick={() => setTab(t)}
                    className="relative text-xs px-3 py-1.5 rounded-full font-medium transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0f766e] min-h-[32px]"
                    style={{ color: tab === t ? "#fff" : "rgba(156,163,175,0.8)", background: tab === t ? "transparent" : "transparent" }}>
                    {tab === t && <motion.span layoutId="tab-pill" className="absolute inset-0 rounded-full" style={{ background: "linear-gradient(135deg, #0f766e, #0d5f58)" }} transition={{ type: "spring", bounce: 0.2, duration: 0.4 }} aria-hidden="true" />}
        ...
        <motion.div initial={reduced ? {} : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1, ease: SPRING }} className="mt-16 sm:mt-20 pt-10 border-t border-gray-200 dark:border-gray-800">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-4">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.5, ease: SPRING }} className="text-center">
              <p className="text-4xl sm:text-5xl font-black tracking-tight font-mono" style={{ color: "#0f766e", letterSpacing: "-0.03em" }}>100%</p>
