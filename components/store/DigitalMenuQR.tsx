...
export default function DigitalMenuQR() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-32">
      {/* Header */}
      <header className="sticky top-0 z-20 bg-[#2d6a4f] text-white px-4 py-4 shadow-lg">
        <h1 className="text-2xl font-bold tracking-tight">Menu Digital</h1>
        <p className="text-sm text-white/70">Bodega San Martin — Pucallpa</p>
              ...
              return (
                ...
                >
                  {product.image ? (
                    ...
                    <div className="w-full h-32 bg-[#2d6a4f]/10 flex items-center justify-center">
                      <span className="text-4xl text-[#2d6a4f]/30 font-black">
                        {product.name[0]}
                      </span>
                    </div>
                  )}
                  <div className="p-3">
                    ...
                    <p className="text-[#2d6a4f] dark:text-[#52b788] font-bold mt-1">
                      {fmt(product.price)}
                      {product.unit ? (
                    ...
                    {inCart ? (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <button
                          onClick={() => changeQty(product.id, 1)}
                          className="w-7 h-7 rounded-full bg-[#2d6a4f] text-white flex items-center justify-center"
                          aria-label="Agregar"
                        >
                      ...
                      <button
                        ...
                        className="mt-2 w-full py-1.5 rounded-xl bg-[#2d6a4f] text-white text-xs font-semibold flex items-center justify-center gap-1"
                      >
                        <Plus className="w-3 h-3" />
                        Agregar
                      </button>
      ...
      {totalItems > 0 && (
        <button
          ...
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 bg-[#2d6a4f] text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 font-bold text-base"
        >
              ...
              {cart.map((item) => (
                <div key={item.id} className="py-3 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 font-bold text-[#2d6a4f] dark:text-[#52b788] text-sm w-20 text-right">
