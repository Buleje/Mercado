  ...
  return (
    <div className="hidden sm:block overflow-x-auto rounded-xl border border-[var(--border-card-border,theme(colors.gray.200))]">
      <table className="w-full text-sm text-left text-[var(--text-foreground)]">
        <thead>
          <tr className="border-b border-[var(--border-card-border,theme(colors.gray.200))] bg-[#2d6a4f]/10 text-[#2d6a4f] dark:bg-[#2d6a4f]/20 dark:text-emerald-300">
            {columns.map((col) => (
              <th
                key={col.key}
            ...
            : data.map((item, idx) => (
                <tr
                  ...
                  className={`
                    ...
                    ${idx % 2 === 0 ? "bg-[var(--bg-card,white)] dark:bg-[var(--bg-card,#1a1a1a)]" : "bg-[#2d6a4f]/[0.03] dark:bg-[#2d6a4f]/[0.06]"}
                    ${onRowClick ? "cursor-pointer hover:bg-[#2d6a4f]/10 dark:hover:bg-[#2d6a4f]/20 transition-colors" : ""}
                  `}
                >
                  {columns.map((col) => (
                    <td
      ...
      {data.map((item) => (
        <div
          ...
          className={`
            ...
            p-4 space-y-2
            ${onRowClick ? "cursor-pointer active:bg-[#2d6a4f]/10 transition-colors" : ""}
          `}
        >
          {visibleCols.map((col) => (
            <div
