...
export default function BannerPreviewStudio({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[100] flex flex-col bg-[#0c1015] text-white"
    >
      {/* ── Top bar ───────────────────────────────────────────────── */}
      <header className="shrink-0 border-b border-white/10 bg-black/60 px-4 sm:px-6 py-3 flex items-center gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[length:var(--ts-2xs)] text-white/60 leading-tight mt-0.5 truncate">
              {slotLabel ? `${slotLabel} · ` : ""}
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {editable && (
            <ModeBtn active={mode === "edit"} onClick={() => setMode("edit")} icon={<Edit3 className="h-3.5 w-3.5" />} label="Editar" hint="1" />
          )}
          <ModeBtn active={mode === "solo"} onClick={() => setMode("solo")} icon={<Monitor className="h-3.5 w-3.5" />} label="Solo" hint="2" />
          <ModeBtn active={mode === "show"} onClick={() => setMode("show")} icon={<Layers className="h-3.5 w-3.5" />} label="Presentación" hint="3" />
          <button
            type="button"
            onClick={onClose}
            className="ml-2 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20 transition-colors"
            aria-label="Cerrar"
          >
            <X className="h-4 w-4" />
          </button>
...
function EditMode({
  ...
  return (
    <div className="flex-1 min-h-0 grid" style={{ gridTemplateColumns: `${leftOpen ? "240px " : "44px "} 1fr ${rightOpen ? "320px" : "44px"}` }}>
      {/* ── LEFT: banner list ─────────────────────────────────────── */}
      <aside className="border-r border-white/10 bg-black/40 overflow-hidden flex flex-col">
        <button
          type="button"
      ...
      {/* ── CENTER: canvas ────────────────────────────────────────── */}
      <section className="relative flex flex-col min-h-0 bg-[radial-gradient(circle_at_center,#1f2937_0%,#0c1015_100%)]">
        {/* Toolbar */}
        <div className="shrink-0 border-b border-white/10 bg-black/40 px-3 py-2 flex items-center gap-2 overflow-x-auto">
      ...
      {/* ── RIGHT: tools panel ─────────────────────────────────────── */}
      <aside className="border-l border-white/10 bg-black/40 overflow-hidden flex flex-col">
        <button
          type="button"
        ...
        {rightOpen && current && (
          <>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-3">
              ...
              {tab === "promo" && <PromoTab banner={current} onPatch={patch} uploadFolder={uploadFolder} />}
              {tab === "state" && (
                <StateTab
                  banner={current}
                  index={idx}
...
function SoloMode({
  ...
  return (
    <>
      <div className="flex-1 min-h-0 flex items-center justify-center p-4 sm:p-8 overflow-auto bg-white">
      ...
      <footer className="shrink-0 border-t border-white/10 bg-black/60 px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
...
function ShowMode({
  ...
  return (
    <>
      <div className="flex-1 min-h-0 flex items-center justify-center p-4 sm:p-8 overflow-auto bg-[#0c1015]">
      ...
      <footer className="shrink-0 border-t border-white/10 bg-black/60 px-4 sm:px-6 py-3 space-y-3">
...
function PromoTab({ banner, onPatch, uploadFolder }: { banner: StudioBanner; onPatch: (p: Partial<StudioBanner>) => void; uploadFolder: string }) {
  ...
  const setPromo = (p: Partial<StudioPromoEmbed>) => onPatch({ promo: { ...promo, ...p } });
  return (
    <div className="space-y-3 text-white">
      <Section title="Imagen del producto">
        <div className="bg-white/5 rounded-lg p-2">
          <ImageUploader
            ...
            onChange={(url) => setPromo({ productImage: url })}
