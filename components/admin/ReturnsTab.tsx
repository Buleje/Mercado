  ...
  return (
    <div className="space-y-3 sm:space-y-6">
          ...
          {!selectedSale ? (
            <div className="space-y-3">
              ...
              <div className="max-h-60 overflow-y-auto space-y-2">
                {sales.map(s => (
                  <button key={s.id} onClick={() => selectSale(s)} className="w-full text-left p-3 border border-gray-200 dark:border-card-border rounded-lg hover:bg-gray-50 dark:hover:bg-surface transition">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea value={reason} onChange={e => setReason(e.target.value)} rows={2} placeholder="Producto dañado, vencido, etc." className="w-full mt-1 px-3 py-2 border border-gray-200 dark:border-card-border rounded-lg bg-white dark:bg-surface text-sm" />
              </div>
              ...
              <div>
                ...
                <button type="button" onClick={() => photoInputRef.current?.click()} className="mt-1 flex flex-wrap items-center gap-2 px-3 py-2 border border-dashed border-gray-300 dark:border-card-border rounded-lg text-sm text-gray-500 dark:text-muted hover:border-primary hover:text-primary transition w-full justify-center">
              ...
              <div className="space-y-2">
                ...
                <input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder="Teléfono del cliente" className="w-full px-3 py-2 border border-gray-200 dark:border-card-border rounded-lg bg-white dark:bg-surface text-sm" />
                {customerPhone.trim() && (
      ...
      <div className="space-y-3">
        ...
        {returns.map(r => (
          <div key={r.id} className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-xl p-4">
            <div className="flex flex-wrap justify-between items-start gap-3">
              ...
              {r.photoUrl && (
                <Image src={r.photoUrl} alt="Evidencia" width={64} height={64} className="object-cover rounded-lg border border-gray-200 dark:border-card-border shrink-0 cursor-pointer" onClick={() => window.open(r.photoUrl)} />
              )}
