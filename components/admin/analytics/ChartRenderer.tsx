      ...
      {data.map((item, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-0.5" title={`${item.label}: ${formatValue(item.value)}`}>
            {showValues && pct > 15 && (
              <span className="text-[length:var(--ts-2xs)] font-bold text-[var(--text-secondary)] dark:text-muted">{formatValue(item.value)}</span>
            )}
            <div
...
export function GaugeChart({ value, size = 120, label, color, className }: GaugeChartProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-1", className)}>
      ...
      {label && <span className="text-[length:var(--ts-2xs)] text-[var(--text-secondary)] dark:text-muted font-medium">{label}</span>}
    </div>
  );
}
        ...
        {computed.map((item, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center relative" style={{ height: "100%" }}>
              ...
              <span className="absolute bottom-0 text-[length:var(--ts-2xs)] text-[var(--text-tertiary)] dark:text-muted truncate max-w-full text-center">
                {item.label}
              </span>
            </div>
          );
