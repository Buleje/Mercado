    ...
    <div className="flex flex-col gap-4 text-sm">
      ...
      <div className="flex gap-2">
        <div className="flex-1 rounded-lg bg-gray-50 dark:bg-gray-800 border border-[var(--rule-base)] p-2 text-center">
          <p className="text-[length:var(--ts-2xs)] text-gray-500 dark:text-gray-400">Entradas</p>
          <p className="text-sm font-bold text-[#00B4A6] dark:text-[#2dd4bf]">
            {fmt(initialBalance + cashSales + digitalSales)}
          </p>
        </div>
        <div className="flex-1 rounded-lg bg-gray-50 dark:bg-gray-800 border border-[var(--rule-base)] p-2 text-center">
          <p className="text-[length:var(--ts-2xs)] text-gray-500 dark:text-gray-400">Salidas</p>
        ...
        >
          <p className="text-[length:var(--ts-2xs)] text-gray-500 dark:text-gray-400">Saldo final</p>
          <p
            className={cn(
              "text-sm font-bold",
      ...
      {/* Waterfall chart */}
      <div className="rounded-lg border border-[var(--rule-base)] bg-gray-50 dark:bg-gray-800/50 p-3">
        ...
        <div className="flex gap-1.5 mt-1">
          {steps.map((bar) => (
            <div key={bar.id} className="flex-1 text-center">
              <p className="text-[length:var(--ts-2xs)] leading-tight text-gray-600 dark:text-gray-300 font-medium">
                {bar.label}
              </p>
              <p className="text-[length:var(--ts-2xs)] leading-tight text-gray-400 dark:text-gray-500">
                {bar.sublabel}
              </p>
            </div>
          ))}
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 justify-center text-[length:var(--ts-2xs)] text-gray-500 dark:text-gray-400">
      ...
      <div className="flex flex-col gap-1">
        {steps.filter((s) => !s.isFinal).map((bar) => (
          <div
            key={bar.id}
            className="flex items-center justify-between px-2 py-1 rounded bg-gray-50 dark:bg-gray-800 border border-[var(--rule-base)] text-xs"
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[length:var(--ts-2xs)] text-gray-400 dark:text-gray-500">{bar.sublabel}</p>
              </div>
            </div>
            <span
              className="font-bold"
