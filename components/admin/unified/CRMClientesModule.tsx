...
const _DASH_COLORS = ["#0f766e","#f97316","#457b9d","#e63946","#9b5de5","#14b8a6","#264653","#6b705c"];
    ...
    <div className="space-y-6 animate-pulse">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-gray-100 dark:bg-card rounded-2xl h-20" />
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-100 dark:bg-card rounded-2xl h-72" />
        <div className="bg-gray-100 dark:bg-card rounded-2xl h-72" />
      </div>
      <div className="bg-gray-100 dark:bg-card rounded-2xl h-64" />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-100 dark:bg-card rounded-2xl h-72" />
        <div className="bg-gray-100 dark:bg-card rounded-2xl h-72" />
      </div>
...
function ClientesDashboard({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  if (customers.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Sin clientes registrados</h3>
        <p className="text-sm text-gray-500 mt-1">Importa o registra tu primer cliente para ver el dashboard</p>
        {onNavigate && (
          <button onClick={() => onNavigate("importar")} className="mt-4 px-4 py-2 bg-[#0f766e] text-white rounded-xl text-sm hover:bg-[#0d5f58] transition-colors">
            Importar Clientes →
          </button>
        )}
      </div>
  ...
  const segmentos = (() => {
    ...
    return [
      { name: "VIP", value: vipCount, color: "#0f766e" },
  ...
  const frecuencia = (() => {
    ...
    return [
      { name: "Frecuente (>10)", value: frecuente, fill: "#0f766e" },
      { name: "Regular (3-10)", value: regular, fill: "#14b8a6" },
  ...
  return (
    <div className="space-y-6">
        ...
        {[
          {
            ...
            value: total,
            icon: Users,
            color: "border-l-4 border-[#0f766e]",
          ...
          return (
            <div
              key={i}
              className={cn(
                "bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-4 shadow-sm hover:shadow-md transition-shadow",
                k.color,
              )}
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-wider font-medium truncate">
                    {k.label}
                  </p>
                  <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {filterSegment && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#0f766e]/10 text-[#0f766e] dark:text-emerald-400 text-xs font-bold">
                  ...
                  <button onClick={() => setFilterSegment(null)} className="hover:bg-[#0f766e]/20 rounded-full p-0.5 transition-colors">
                    <XIcon className="h-3 w-3" />
                  </button>
                </span>
              )}
              <button onClick={() => setExpandedChart("segmentos")} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors" title="Expandir"><Maximize2 className="h-3.5 w-3.5 text-gray-400" /></button>
            </div>
          </div>
          {segmentos.length > 0 ? (
        ...
        <ResponsiveContainer width="100%" height={250}>
          <AreaChart data={newByMonth} margin={{ left: 0, right: 10, top: 10, bottom: 0 }}>
            ...
            <Area
              ...
              dot={{ r: 4, fill: "#0f766e", strokeWidth: 2, stroke: "white" }}
              activeDot={{ r: 6 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      ...
      <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-6 shadow-sm">
        <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Retencion mensual</h3>
        {(() => {
          ...
          return retData.some(d => d.retencion > 0) ? (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={retData} margin={{ left: 0, right: 10, top: 10, bottom: 0 }}>
                ...
                <Tooltip content={<ChartTooltip />} />
                <Line type="monotone" dataKey="retencion" stroke="#0f766e" strokeWidth={2.5} dot={{ r: 4, fill: "#0f766e", strokeWidth: 2, stroke: "white" }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <div className="text-4xl mb-3">📊</div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Sin datos de retencion</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Los datos apareceran cuando tengas clientes recurrentes</p>
            </div>
          );
        })()}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-red-50 dark:bg-red-950/20 border-l-4 border-red-500 rounded-xl p-4">
        ...
        <div className="bg-amber-50 dark:bg-amber-950/20 border-l-4 border-amber-500 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
      ...
      {expandedChart && (
        <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 p-8 overflow-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Clientes por segmento</h2>
