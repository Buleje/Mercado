    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {[
          ...
          <button key={m.key} onClick={() => setMetric(m.key)} className={cn("flex items-center gap-2 px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg text-sm font-bold transition-colors", metric === m.key ? "bg-[var(--text-primary)] text-white" : "bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-accent")}>
          ...
          {sorted.map((p, i) => (
            <div key={p.id} className="flex flex-wrap items-center gap-3">
              ...
              <div className="w-20 flex items-center gap-1">
                ...
                <span className="text-[length:var(--ts-2xs)] font-bold text-gray-400 w-10 text-right">{p.cumPct.toFixed(0)}%</span>
              </div>
              <span className={cn("text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded-full", p.isTop ? "bg-[var(--surface-sunken)] text-[var(--text-primary)]" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400")}>{p.isTop ? "TOP" : "—"}</span>
            </div>
          ))}
        </div>
        {/* 80% line reference */}
      ...
      <div className="bg-[var(--surface-sunken)] rounded-xl border border-[var(--rule-base)] p-3 sm:p-5">
        <h3 className="font-extrabold text-[var(--text-secondary)] dark:text-[var(--text-primary)] text-sm">Insight Pareto</h3>
