    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        {(["whatsapp", "email", "sms"] as TemplateChannel[]).map(ch => {
          ...
          return (
            <div key={ch} className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-card-border p-4 ">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar plantilla..." className="pl-9 pr-4 py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-gray-900 dark:text-foreground text-sm outline-none focus:border-primary w-56" />
        </div>
        ...
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value as typeof categoryFilter)} className="px-3 py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-sm outline-none focus:border-primary">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {filtered.map(t => {
            ...
            return (
              <div key={t.id} onClick={() => setSelectedId(t.id)} className={cn("bg-white dark:bg-card rounded-xl border p-4 cursor-pointer transition-all hover:shadow-sm", selectedId === t.id ? "border-primary" : "border-gray-200 dark:border-card-border")}>
        ...
        <div className="lg:col-span-1">
          {selected ? (
            <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-card-border p-3 sm:p-5  sticky top-6">
              ...
              <button onClick={() => handleCopy(selected)} className="w-full mt-4 flex flex-wrap items-center justify-center gap-2 px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg bg-primary text-white text-sm font-bold hover:bg-primary/90 transition-colors">
                {copiedId === selected.id ? <><Check className="h-4 w-4" /> Copiado</> : <><Copy className="h-4 w-4" /> Copiar mensaje</>}
              </button>
            </div>
          ) : (
            <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-card-border p-8 text-center">
          ...
          <div className="bg-white dark:bg-card rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-3 sm:px-6 py-5 space-y-4">
              <div>
                ...
                <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="Ej: Confirmación de pedido" className="w-full px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-gray-900 dark:text-foreground text-sm outline-none focus:border-primary" autoFocus />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-foreground mb-1">Canal</label>
                  <select value={newChannel} onChange={e => setNewChannel(e.target.value as TemplateChannel)} className="w-full px-3 py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-sm outline-none focus:border-primary">
                ...
                <div>
                  ...
                  <select value={newCategory} onChange={e => setNewCategory(e.target.value as TemplateCategory)} className="w-full px-3 py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-sm outline-none focus:border-primary">
                    {Object.entries(CATEGORY_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                  </select>
                </div>
              </div>
              {newChannel === "email" && (
                <div>
                  ...
                  <input type="text" value={newSubject} onChange={e => setNewSubject(e.target.value)} placeholder="Asunto del email..." className="w-full px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-sm outline-none focus:border-primary" />
                </div>
              )}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-foreground mb-1">Mensaje</label>
                <textarea value={newBody} onChange={e => setNewBody(e.target.value)} placeholder={"Usa {{variable}} para campos dinámicos...\nEj: Hola {{cliente}}, tu pedido..."} rows={6} className="w-full px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-gray-900 dark:text-foreground text-sm outline-none focus:border-primary resize-none font-mono" />
