    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
              ...
              <div key={t.id} onClick={() => setSelectedId(t.id)} className={cn("bg-white dark:bg-card rounded-xl border p-4 cursor-pointer transition-all hover:shadow-sm", selectedId === t.id ? "border-primary" : "border-[var(--rule-base)] dark:border-card-border")}>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-surface text-[var(--text-secondary)] dark:text-muted text-xs font-semibold">{CATEGORY_LABELS[t.category]}</span>
                  </div>
                </div>
                <p className="text-xs text-[var(--text-secondary)] dark:text-muted line-clamp-2 mb-3 whitespace-pre-line">{t.body.replace(/\{\{(\w+)\}\}/g, "[$1]")}</p>
          ...
          {filtered.length === 0 && <div className="text-center py-12"><MessageSquare className="h-12 w-12 text-[var(--text-tertiary)] mx-auto mb-3" /><p className="text-[var(--text-tertiary)] font-semibold">No hay plantillas</p></div>}
        </div>
