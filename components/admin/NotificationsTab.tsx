              ...
              {logs.map(l => (
                <div key={l.id} className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl px-2 sm:px-4 py-2 sm:py-3 space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[length:var(--ts-2xs)] text-gray-400">{new Date(l.createdAt).toLocaleString("es-PE")}</span>
                  </div>
                  <p className="text-xs text-gray-500 line-clamp-2">{l.message}</p>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[length:var(--ts-2xs)] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-surface text-gray-500">{TYPE_LABELS[l.type] ?? l.type}</span>
                    {l.orderId && <span className="text-[length:var(--ts-2xs)] text-gray-400">#{l.orderId.slice(0, 8)}</span>}
                  </div>
                </div>
              ))}
            </div>
