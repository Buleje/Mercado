  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
        <div>
          <SectionTitle className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Filtros Guardados</SectionTitle>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)] mt-1">{filters.length}</p>
        </div>
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
        ...
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)] mt-1">{new Set(filters.map(f => f.module)).size}</p>
        </div>
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar filtros..." className="pl-9 pr-4 py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-[var(--text-primary)] dark:text-[var(--text-primary)] text-sm outline-none focus:border-primary w-56" />
        </div>
        <select value={moduleFilter} onChange={e => setModuleFilter(e.target.value)} className="px-3 py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-sm outline-none focus:border-primary">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-3 sm:p-5 animate-pulse">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
        {filtered.map(f => (
          <div key={f.id} className={cn("bg-[var(--surface-raised)] rounded-xl border p-3 sm:p-5 transition-shadow hover:shadow-[var(--shadow-sm)]", f.isDefault ? "border-primary/50 dark:border-primary/50" : "border-[var(--rule-base)] dark:border-[var(--rule-base)]")}>
            ...
            <div className="space-y-1.5 mb-3">
              {f.conditions.map((c, i) => (
                <div key={i} className="flex items-center gap-1.5 text-xs">
                  <span className="font-semibold text-[var(--text-primary)] dark:text-[var(--text-primary)]">{c.field}</span>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-[var(--rule-soft)] dark:border-[var(--rule-base)]">
        ...
        <div className="modal-backdrop p-4" onClick={() => setShowNew(false)}>
          <div className="bg-[var(--surface-raised)] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-3 sm:px-6 py-4 border-b border-[var(--rule-soft)] dark:border-[var(--rule-base)]">
              <CardTitle className="font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Nuevo filtro guardado</CardTitle>
                ...
                <div className="space-y-2">
                  {newConditions.map((cond, i) => (
                    <div key={i} className="flex flex-wrap items-center gap-2">
                      <select value={cond.field} onChange={e => updateCondition(i, "field", e.target.value)} className="flex-1 px-2 py-2 rounded-lg border border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-xs outline-none focus:border-primary">
                        <option value="">Campo...</option>
                        {(FIELDS[newModule] || []).map(f => <option key={f} value={f}>{f}</option>)}
                      </select>
                      <select value={cond.operator} onChange={e => updateCondition(i, "operator", e.target.value)} className="w-28 px-2 py-2 rounded-lg border border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-xs outline-none focus:border-primary">
                        {OPERATORS.map(o => <option key={o} value={o}>{o}</option>)}
                      </select>
                      <input type="text" value={cond.value} onChange={e => updateCondition(i, "value", e.target.value)} placeholder="Valor" className="flex-1 px-2 py-2 rounded-lg border border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-xs outline-none focus:border-primary" />
