...
const STATUS_META: Record<PolicyStatus, { label: string; color: string; bg: string; icon: typeof CheckCircle2 }> = {
  vigente:    { label: "Vigente",    color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]", icon: CheckCircle2 },
  "por-vencer": { label: "Por Vencer", color: "text-[var(--data-warning)]",   bg: "bg-[var(--data-warning-100)] dark:bg-[var(--data-warning)]/30",   icon: Clock },
  vencida:    { label: "Vencida",    color: "text-[var(--data-error)]",     bg: "bg-[var(--data-error-100)] dark:bg-[var(--data-error)]/30",       icon: XCircle },
};

const CLAIM_STATUS: Record<ClaimStatus, { label: string; color: string; bg: string }> = {
  abierto:     { label: "Abierto",     color: "text-[var(--data-success)]",    bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
  "en-revision": { label: "En Revisión", color: "text-[var(--data-warning)]",   bg: "bg-[var(--data-warning-100)] dark:bg-[var(--data-warning)]/30" },
  aprobado:    { label: "Aprobado",    color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
  rechazado:   { label: "Rechazado",   color: "text-[var(--data-error)]",     bg: "bg-[var(--data-error-100)] dark:bg-[var(--data-error)]/30" },
};
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Pólizas vigentes", value: String(stats.vigentes), color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
          { label: "Prima mensual total", value: fmt(stats.monthlyTotal), color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
          { label: "Siniestros abiertos", value: String(stats.claimsOpen), color: "text-[var(--data-warning)]", bg: "bg-[var(--data-warning-50)] dark:bg-amber-950/30" },
      ...
      {/* Alerts */}
      {alerts.length > 0 && (
        <div className="bg-[var(--data-warning-50)] dark:bg-amber-950/20 border border-[var(--data-warning)] dark:border-[var(--data-warning)] rounded-xl p-4 space-y-2">
          <p className="font-bold text-[var(--data-warning)] dark:text-[var(--data-warning)] text-sm flex items-center gap-1.5"><AlertTriangle className="h-4 w-4" /> Alertas de vencimiento</p>
          {alerts.map(a => (
            <div key={a.id} className="flex items-center justify-between text-xs">
              <span className="text-[var(--data-warning)] dark:text-[var(--data-warning)]">{TYPE_LABELS[a.type]} — {a.insurer} ({a.policyNumber})</span>
              <span className={cn("font-bold", a.status === "vencida" ? "text-[var(--data-error)]" : "text-[var(--data-warning)]")}>
                  ...
                  {filtered.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-gray-50/50 dark:hover:bg-surface/30 transition-colors">
                        ...
                        <td className="px-2 sm:px-4 py-2 sm:py-3"><button onClick={() => setDetail(p)} className="p-1.5 rounded-lg text-[var(--text-tertiary)] hover:text-[var(--data-success)] hover:bg-[var(--accent-soft)] dark:hover:bg-[var(--accent-muted)]"><Eye className="h-3.5 w-3.5" /></button></td>
                      </tr>
                    );
                  })}
                </tbody>
                ...
                {claims.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-gray-50/50 dark:hover:bg-surface/30 transition-colors">
                      ...
                      <td className="px-2 sm:px-4 py-2 sm:py-3"><button onClick={() => setClaimDetail(c)} className="p-1.5 rounded-lg text-[var(--text-tertiary)] hover:text-[var(--data-success)] hover:bg-[var(--accent-soft)] dark:hover:bg-[var(--accent-muted)]"><Eye className="h-3.5 w-3.5" /></button></td>
                    </tr>
                  );
                })}
              </tbody>
