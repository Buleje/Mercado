...
/* ── Seed Data ── */
type Product = { name: string; unitPrice: number; unitCost: number; monthlySales: number };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={() => exportToCSV(PRODUCTS.map(p => ({ Producto: p.name, Precio: p.unitPrice, Costo: p.unitCost, Margen: (p.unitPrice - p.unitCost).toFixed(2), VentasMes: p.monthlySales })), "break-even")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-bold hover:bg-primary/90">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button onClick={() => { setSelectedProduct("general"); setCustomPrice(""); setCustomCost(""); }} className={cn("px-4 py-2.5 rounded-xl text-sm font-bold transition-colors", selectedProduct === "general" ? "bg-red-500 text-white" : "bg-white dark:bg-card border border-gray-200 dark:border-card-border text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-accent")}>
          Vista General
        </button>
        {PRODUCTS.map((p, i) => (
          <button key={i} onClick={() => { setSelectedProduct(i); setCustomPrice(""); setCustomCost(""); }} className={cn("px-4 py-2.5 rounded-xl text-sm font-bold transition-colors", selectedProduct === i ? "bg-red-500 text-white" : "bg-white dark:bg-card border border-gray-200 dark:border-card-border text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-accent")}>
      ...
      <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <input value={customFixed} onChange={e => setCustomFixed(e.target.value.replace(/[^\d.]/g, ""))} placeholder="Sobrescribir total fijo" className="ml-auto px-4 py-2 rounded-xl border-2 border-gray-200 dark:border-card-border bg-white dark:bg-card text-sm outline-none focus:border-primary w-48" />
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              ...
              <p className="text-2xl font-extrabold text-gray-900 dark:text-foreground mt-1">{fmt(generalAnalysis.totalRevenue)}</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              <p className="text-xs font-semibold text-gray-500 dark:text-muted uppercase">Punto de Equilibrio</p>
              <p className="text-2xl font-extrabold text-red-600 dark:text-red-400 mt-1">{fmt(generalAnalysis.breakEvenRevenue)}</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              <p className="text-xs font-semibold text-gray-500 dark:text-muted uppercase">Utilidad Mensual</p>
              <p className={cn("text-2xl font-extrabold mt-1", generalAnalysis.currentProfit >= 0 ? "text-emerald-600" : "text-red-600")}>{fmt(generalAnalysis.currentProfit)}</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              <p className="text-xs font-semibold text-gray-500 dark:text-muted uppercase">Margen de Seguridad</p>
              <p className={cn("text-2xl font-extrabold mt-1", generalAnalysis.safetyMargin >= 20 ? "text-emerald-600" : "text-amber-600")}>{generalAnalysis.safetyMargin.toFixed(1)}%</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" step="0.10" value={customPrice || productAnalysis.product.unitPrice} onChange={e => setCustomPrice(e.target.value)} className="w-full mt-1 px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-card-border bg-white dark:bg-card text-sm font-bold outline-none focus:border-primary" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500">Costo Unitario (S/)</label>
                <input type="number" step="0.10" value={customCost || productAnalysis.product.unitCost} onChange={e => setCustomCost(e.target.value)} className="w-full mt-1 px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-card-border bg-white dark:bg-card text-sm font-bold outline-none focus:border-primary" />
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              ...
              <p className="text-2xl font-extrabold text-blue-600 dark:text-blue-400 mt-1">{fmt(productAnalysis.contrib)}</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              <p className="text-xs font-semibold text-gray-500 dark:text-muted uppercase">Punto Equilibrio (und)</p>
              <p className="text-2xl font-extrabold text-red-600 dark:text-red-400 mt-1">{productAnalysis.breakEvenUnits === Infinity ? "∞" : productAnalysis.breakEvenUnits.toLocaleString()}</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              <p className="text-xs font-semibold text-gray-500 dark:text-muted uppercase">Ventas Actuales</p>
              <p className="text-2xl font-extrabold text-gray-900 dark:text-foreground mt-1">{productAnalysis.monthlySales.toLocaleString()} und</p>
            </div>
            <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
              ...
              <p className={cn("text-2xl font-extrabold mt-1", productAnalysis.currentProfit >= 0 ? "text-emerald-600" : "text-red-600")}>{fmt(productAnalysis.currentProfit)}</p>
            </div>
          </div>
          {/* Visual bar */}
          <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-5">
