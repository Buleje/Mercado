    ...
    <div className="rounded-xl border border-[var(--rule-base)] dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold text-[var(--text-primary)] dark:text-zinc-300">Tarifas de Delivery por Zona</span>
        </div>
      </div>
      <div className="space-y-1.5">
        {zones.map((z, i) => (
          <div key={z.zona + i} className="flex items-center gap-3 bg-gray-50 dark:bg-zinc-700/50 rounded-lg px-3 py-2">
            <span className="flex-1 text-xs font-bold text-[var(--text-primary)] dark:text-zinc-200">{z.zona}</span>
            <span className={cn(
      ...
      {addingZone ? (
        <div className="mt-2 flex items-center gap-2">
          <input type="text" placeholder="Nombre zona" value={newZone.zona} onChange={e => setNewZone({...newZone, zona: e.target.value})} className="flex-1 text-xs border border-[var(--rule-base)] dark:border-zinc-600 rounded-lg px-2 py-1.5 bg-white dark:bg-zinc-900 text-[var(--text-primary)] dark:text-zinc-100" />
          <div className="flex items-center gap-1">
            <span className="text-xs text-[var(--text-tertiary)]">S/</span>
            <input type="number" min={0} value={newZone.tarifa} onChange={e => setNewZone({...newZone, tarifa: Number(e.target.value) || 0})} className="w-16 text-xs border border-[var(--rule-base)] dark:border-zinc-600 rounded-lg px-2 py-1.5 bg-white dark:bg-zinc-900 text-[var(--text-primary)] dark:text-zinc-100" />
          </div>
          ...
          <button onClick={() => setAddingZone(false)} className="text-xs text-[var(--text-tertiary)]">&times;</button>
        </div>
      ) : (
        <button onClick={() => setAddingZone(true)} className="mt-2 w-full py-1.5 rounded-lg border border-dashed border-[var(--rule-base)] dark:border-zinc-600 text-xs font-bold text-[var(--text-tertiary)] hover:text-primary hover:border-primary/40 transition-colors">
          + Agregar zona
        </button>
      )}
      <p className="text-[length:var(--ts-2xs)] text-[var(--text-tertiary)] dark:text-zinc-500 mt-2">Estas tarifas se aplican automaticamente al checkout segun la zona del cliente.</p>
    </div>
  );
}
