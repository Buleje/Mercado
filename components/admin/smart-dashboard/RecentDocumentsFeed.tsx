        ...
        {docs.map((d, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 text-sm">
              <span className="text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded bg-gray-100 dark:bg-zinc-700 text-gray-600 dark:text-zinc-300 shrink-0">{DOC_ICONS[d.type] ?? d.type}</span>
              <span className="text-gray-700 dark:text-zinc-200 font-medium truncate">{d.number}</span>
              <span className="text-[length:var(--ts-2xs)] text-gray-400 dark:text-zinc-500 capitalize">{d.status}</span>
              <span className="text-[length:var(--ts-2xs)] text-gray-400 dark:text-zinc-500 ml-auto shrink-0">{timeLabel}</span>
            </div>
          );
        })}
      </div>
