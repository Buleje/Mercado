...
/**
 * LothTraceTimeline — las seis secciones SERFOR de un árbol, con sus FECHAS y
 * sus líneas.
 *
 * Vivía al pie de `LothTraceDetalle`, que pasaba de 300 líneas. Dos cosas que
 * justifican media pantalla:
 *  · las fechas de cada etapa (el libro se fiscaliza por plazos, no sólo por
 *    volúmenes) y los días que tardó en moverse;
 *  · los códigos no son texto muerto: la troza abre su cadena de custodia y la
 *    GTF lleva a su guía.
 *
 * Las líneas de producto sin código de troza se marcan «por especie»: son una
 * atribución estimada, no un hecho de ESTE árbol.
 */

import { ExternalLink, MapPin } from "@buleje/design-system/icons";
import type { LothEntryDTO } from "@/lib/forestal/loth-constants";
import type { TraceOperation } from "@/lib/forestal/loth-trace";
import { fmtM3 } from "@/lib/forestal/cubicacion-formato";
import { etapasDe, fmtDias, fmtFechaLarga, plural, type TraceNav } from "./loth-trace-ui";

const num = (v: string | null, dp = 4) => (v == null ? "—" : Number(v).toFixed(dp));

// ─── timeline detallado de las 6 secciones ───────────────────────────────────
/* Tinte con alpha y su `dark:`: los `-100` sólidos de antes eran un parche claro
   con texto oscuro encima en el tema oscuro. */
const TROZA_PILL: Record<string, { label: string; cls: string }> = {
  despachada: { label: "despachada", cls: "bg-[var(--data-info-500)]/15 text-[var(--data-info-700)] dark:text-[var(--data-info-500)]" },
  consumida: { label: "consumida", cls: "bg-[var(--data-success-500)]/15 text-[var(--data-success-700)] dark:text-[var(--data-success-500)]" },
  patio: { label: "en patio", cls: "bg-[var(--data-warning-500)]/15 text-[var(--data-warning-700)] dark:text-[var(--data-warning-500)]" },
};

function TrozaPill({ estado }: { estado: "despachada" | "consumida" | "patio" }) {
  const p = TROZA_PILL[estado];
  return <span className={`rounded px-1.5 py-0.5 text-xs font-bold ${p.cls}`}>{p.label}</span>;
}

export default function LothTraceTimeline({ op, nav }: { op: TraceOperation; nav?: TraceNav }) {
      ...
      <div className="text-[var(--text-tertiary)]">
        ...
        <span className="ml-1">
          ...
          <span className={op.trozasEnPatio > 0 ? "font-bold text-[var(--data-warning-700)] dark:text-[var(--data-warning-500)]" : ""}>
            {op.trozasEnPatio} en patio{op.patioVolM3 > 0 ? ` (${fmtM3(op.patioVolM3)} m³)` : ""}
          </span>
        </span>
      </div>
          ...
          return (
            <div key={t.id}>
              <span className="inline-flex items-center gap-1.5">
                ...
                {t.isRama && <span className="text-xs text-[var(--text-tertiary)]">(rama)</span>}
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <div className="flex flex-wrap items-center gap-x-2">
                ...
                {demora != null && demora > 0 && (
                  <span className="text-xs text-[var(--text-tertiary)]">· {fmtDias(demora)} después</span>
                )}
              </div>
...
/** La línea no dice de qué troza salió: se le atribuye a este árbol por especie. */
function PorEspecie() {
  return (
    <span
      className="ml-1.5 rounded border border-dashed border-[var(--rule-strong)] px-1 text-xs font-semibold text-[var(--text-secondary)]"
      title="La línea no declara de qué troza salió: se reparte por especie entre los árboles que fueron al aserrío"
    >
      por especie
    </span>
  );
