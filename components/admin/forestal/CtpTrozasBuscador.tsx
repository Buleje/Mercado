 ...
 *
 * Pregunta al SERVIDOR y no a la lista del patio: el patio viene acotado a 5.000
 * piezas y un aserradero grande podría no tener en memoria justo la que le están
 * pidiendo. Acá el "no está" es un no de verdad.
 *
...
type Troza = {
  ...
  /** Los DOS extremos del tronco: es como los publica SERFOR y con lo que se cubica. */
  d1Cm: number | null;
  d2Cm: number | null;
  /** El código que este centro le marcó a la pieza al recibirla. */
  codigoPlanta: string | null;
  /** En qué lote de aserrío está apartada, si está en alguno (ADR-334). */
  loteAserrioCode: string | null;
  loteAserrioStatus: string | null;
  parcela: string | null;
...
export default function CtpTrozasBuscador() {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--rule-soft)] text-left text-[length:var(--ts-2xs)] uppercase tracking-[var(--ls-wider)] text-[var(--text-tertiary)]">
                  ...
                  {/* El de planta al lado del del bosque: en el patio se busca por
                      el que está marcado en la pieza, no por el de SERFOR. */}
                  <th className="px-4 py-2 font-bold">Código planta</th>
                  <th className="px-4 py-2 font-bold">Especie</th>
                  {/* Dónde está la pieza AHORA: apartada en un lote o suelta en
                      el patio. Es la mitad de la pregunta «¿esta troza entró
                      acá?» que antes no se podía contestar (ADR-334). */}
                  <th className="px-4 py-2 font-bold">Lote</th>
                  <th className="px-4 py-2 text-right font-bold" title="Diámetro del extremo mayor">D1 (cm)</th>
                  <th className="px-4 py-2 text-right font-bold" title="Diámetro del extremo menor">D2 (cm)</th>
                  <th className="px-4 py-2 text-right font-bold">Largo (m)</th>
                  <th className="px-4 py-2 text-right font-bold" title="Promedio de D1 y D2 — el que se usa para cubicar">Diám. (cm)</th>
                  ...
                  <tr key={t.id} className="border-b border-[var(--rule-soft)] last:border-0">
                    ...
                    <td className="px-4 py-2.5">
                      {t.loteAserrioCode ? (
                        <span
                          title={
                            t.loteAserrioStatus === "abierto"
                              ? "Apartada en un lote que todavía no entró a la sierra"
                              : "El lote con el que se cargó la sierra"
                          }
                          className={`inline-block rounded-lg px-1.5 py-0.5 font-mono text-xs font-bold ${
                            t.loteAserrioStatus === "abierto"
                    ...
                    <td className="px-4 py-2.5 text-right font-mono tabular-nums text-[var(--text-secondary)]">
                      {t.d1Cm != null ? t.d1Cm.toFixed(0) : "—"}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono tabular-nums text-[var(--text-secondary)]">
                      {t.d2Cm != null ? t.d2Cm.toFixed(0) : "—"}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono tabular-nums text-[var(--text-secondary)]">
          ...
          <ul className="divide-y divide-[var(--rule-soft)] sm:hidden">
            {g.trozas.map((t) => (
              <li key={t.id} className="px-4 py-3">
                ...
                {t.loteAserrioCode && (
                  <p className="mt-0.5 font-mono text-xs font-bold text-[var(--accent-ink)] dark:text-[var(--accent)]">
                    En el lote {t.loteAserrioCode}
                  </p>
                )}
                <p className="mt-0.5 font-mono text-xs text-[var(--text-tertiary)]">
                  {t.codigoPlanta && <>planta {t.codigoPlanta} · </>}
                  D1 {t.d1Cm != null ? t.d1Cm.toFixed(0) : "—"} · D2 {t.d2Cm != null ? t.d2Cm.toFixed(0) : "—"} cm ·{" "}
                  largo {t.largoM != null ? `${t.largoM.toFixed(2)} m` : "—"}
                  {t.diametroCm != null && ` · Ø ${t.diametroCm.toFixed(1)} cm`}
                </p>
              </li>
            ))}
          </ul>
