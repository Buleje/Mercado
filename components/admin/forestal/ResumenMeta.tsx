      ...
      <div className="mb-1 flex items-baseline justify-between gap-2 text-sm">
        <span className={`font-bold ${estado.cumple
          ...
          {estado.cumple
            ...
            : `Falta: ${n1(estado.actual)}% de ${meta.tipo} (${n1(estado.faltanPuntos)} puntos, ${fmtPt(estado.faltanPt)} PT)`}
        </span>
      ...
      {tendencia.puntos.length > 1 && (
        <div className="mt-4">
          <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
            ...
            <span className={`inline-flex items-center gap-1 text-sm font-bold ${tendencia.deltaPctMeta >= 0
              ? "text-[var(--data-success-700)] dark:text-[var(--data-success-500)]"
              : "text-[var(--data-warning-700)] dark:text-[var(--data-warning-500)]"}`}>
          ...
          <ul className="flex items-end gap-1.5" style={{ height: 72 }}>
            {tendencia.puntos.map((p) => (
              <li key={p.id} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1" title={`${p.nombre} · ${n1(p.pctMeta)}% ${meta.tipo} · ${fmtPt(p.pieTablar)} PT`}>
