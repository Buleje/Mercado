    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid gap-2 sm:gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-xl border border-gray-200 bg-white p-3 sm:p-5 dark:border-card-border dark:bg-card">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input type="number" min={1} value={form.quantity} onChange={(event) => setForm((prev) => ({ ...prev, quantity: event.target.value }))} placeholder="Cantidad" className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-card-border dark:bg-surface" />
            <select value={form.cause} onChange={(event) => setForm((prev) => ({ ...prev, cause: event.target.value as ShrinkageCause }))} className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-card-border dark:bg-surface">
              {Object.entries(CAUSE_META).map(([key, meta]) => (
                <option key={key} value={key}>{meta.label}</option>
              ))}
            </select>
            <input value={form.reportedBy} onChange={(event) => setForm((prev) => ({ ...prev, reportedBy: event.target.value }))} placeholder="Reportado por" className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-card-border dark:bg-surface" />
            <textarea value={form.notes} onChange={(event) => setForm((prev) => ({ ...prev, notes: event.target.value }))} placeholder="Notas" rows={3} className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm sm:col-span-2 dark:border-card-border dark:bg-surface" />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
          ...
          <div className="col-span-2 rounded-xl border border-gray-200 bg-white p-3 sm:p-5 dark:border-card-border dark:bg-card">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative max-w-xs flex-1">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar producto o categoria..." className="w-full rounded-lg border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm dark:border-card-border dark:bg-card" />
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setFilterCause("todos")} className={cn("rounded-lg px-3 py-2 text-xs font-bold", filterCause === "todos" ? "bg-primary text-white" : "border border-gray-200 bg-white text-gray-600 dark:border-card-border dark:bg-card dark:text-muted")}>Todos</button>
          {(Object.keys(CAUSE_META) as ShrinkageCause[]).map((cause) => (
            <button key={cause} onClick={() => setFilterCause(cause)} className={cn("rounded-lg px-3 py-2 text-xs font-bold", filterCause === cause ? "bg-primary text-white" : "border border-gray-200 bg-white text-gray-600 dark:border-card-border dark:bg-card dark:text-muted")}>
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-3 sm:p-6 dark:border-card-border dark:bg-card">
