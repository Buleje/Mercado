    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
        ...
        <button
          ...
          className="rounded-xl border-2 border-dashed border-[var(--rule-base)] dark:border-card-border p-4 hover:border-primary/40 hover:bg-gray-50 dark:hover:bg-accent/30 transition-all flex flex-col items-center justify-center gap-2 text-gray-400 dark:text-muted min-h-[100px]"
        >
        ...
        <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-3 sm:p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={accForm.name} onChange={e => setAccForm(p => ({ ...p, name: e.target.value }))} placeholder="BCP Cuenta Corriente" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Banco</label>
              <select value={accForm.bank} onChange={e => setAccForm(p => ({ ...p, bank: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">
                {BANKS.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Número (últimos 4)</label>
              <input type="text" value={accForm.accountNumber} onChange={e => setAccForm(p => ({ ...p, accountNumber: e.target.value }))} placeholder="•••• 0000" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Saldo actual (S/)</label>
              <input type="number" value={accForm.balance} onChange={e => setAccForm(p => ({ ...p, balance: e.target.value }))} placeholder="0.00" min="0" step="0.01" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
            </div>
            <div>
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            <button onClick={() => setShowNewAccount(false)} className="px-2 sm:px-4 py-1.5 sm:py-2 text-sm rounded-lg border border-[var(--rule-base)] dark:border-card-border text-gray-600 dark:text-muted">Cancelar</button>
          ...
          <div className="px-3 sm:px-6 py-4 bg-gray-50 dark:bg-surface/40 border-b border-[var(--rule-soft)] dark:border-card-border">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              <div>
                ...
                <select value={txnForm.accountId} onChange={e => setTxnForm(p => ({ ...p, accountId: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">
                  {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Tipo</label>
                <select value={txnForm.type} onChange={e => setTxnForm(p => ({ ...p, type: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">
              ...
              <div>
                ...
                <input type="text" value={txnForm.description} onChange={e => setTxnForm(p => ({ ...p, description: e.target.value }))} placeholder="Concepto..." className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Monto</label>
                <input type="number" value={txnForm.amount} onChange={e => setTxnForm(p => ({ ...p, amount: e.target.value }))} placeholder="0.00" min="0" step="0.01" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
              </div>
              <div className="flex flex-wrap items-end gap-2">
                ...
                <button onClick={() => setShowNewTxn(false)} className="p-2 rounded-lg border border-[var(--rule-base)] dark:border-card-border text-gray-400 hover:bg-gray-100 dark:hover:bg-accent"><X className="h-4 w-4" /></button>
              </div>
            </div>
          </div>
        )}
