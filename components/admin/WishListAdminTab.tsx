...
import { Heart, Search, Eye, Download, TrendingUp, Loader2, AlertTriangle, Bell, BellOff, Package } from "@buleje/design-system/icons";
...
export default function WishListAdminTab() {
  ...
  if (loading) return (
    <LoadingState message="Cargando listas de deseos..." />
  );
  if (error) return (
    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setFilterType("oos")} className={cn("px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-1", filterType === "oos" ? "bg-primary text-white" : "bg-gray-100 dark:bg-surface text-[var(--text-secondary)] dark:text-muted")}>
              ...
              <div key={l.id} className="bg-white dark:bg-card rounded-xl border border-[var(--rule-base)] dark:border-card-border overflow-hidden">
                <button onClick={() => setExpandedId(expandedId === l.id ? null : l.id)} className="w-full flex items-center justify-between px-4 sm:px-5 py-4 hover:bg-gray-50 dark:hover:bg-surface/50 transition-colors">
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    <span className="text-[length:var(--ts-2xs)] text-[var(--text-tertiary)]">Act: {fmtDate(l.lastUpdated)}</span>
                    ...
                    {l.items.map(i => (
                      <div key={i.productId} className="flex items-center justify-between py-2 px-3 rounded-xl bg-gray-50 dark:bg-surface">
                        ...
                        <div className="flex items-center gap-2 shrink-0 ml-2">
                          ...
                          {!i.inStock && (
                            <button onClick={() => handleNotify(i.productId, i.productName)} disabled={notified.has(i.productId)} className={cn("flex items-center gap-1 text-[length:var(--ts-2xs)] font-bold px-2 py-0.5 rounded-full transition-colors", notified.has(i.productId) ? "text-[var(--text-tertiary)]" : "text-primary hover:underline")}>
