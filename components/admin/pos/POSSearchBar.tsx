...
function stockBadge(stock: number | undefined) {
  ...
  if (stock <= 0)
    return (
      <span className="text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-muted">
        Sin stock
      </span>
    );
  if (stock <= 2)
    return (
      <span className="text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded-full bg-red-50 dark:bg-red-950/30 text-red-500">
        {stock}
      </span>
    );
  if (stock <= 10)
    return (
      <span className="text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/30 text-amber-500">
        {stock}
      </span>
    );
  return (
    <span className="text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/30 text-emerald-500">
      {stock}
    </span>
  );
}
    ...
    <div ref={containerRef} className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          autoComplete="off"
        />
        <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[length:var(--ts-2xs)] bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-muted px-1.5 py-0.5 rounded font-mono">
          F1
        </kbd>
      </div>
