  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-gray-900 dark:text-foreground">Estacionalidad de Productos</h2>
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-xl border border-gray-200 dark:border-card-border overflow-hidden">
            {(["mensual", "productos"] as const).map(v => (
              <button key={v} onClick={() => setView(v)} className={cn("px-4 py-2 text-sm font-semibold transition-colors capitalize", view === v ? "bg-primary text-white" : "text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-surface")}>{v}</button>
            ))}
          </div>
          <button
      ...
      {view === "productos" && (
        <>
          <div className="flex items-center gap-3">
            <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="px-3 py-2.5 rounded-xl border-2 border-gray-200 dark:border-card-border bg-white dark:bg-surface text-gray-900 dark:text-foreground text-sm outline-none focus:border-primary">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-surface border-b border-gray-200 dark:border-card-border">
                  ...
                  <th className="text-left px-4 py-3 text-gray-500 dark:text-muted font-semibold">Meses pico</th>
                  <th className="text-left px-4 py-3 text-gray-500 dark:text-muted font-semibold">Meses bajos</th>
                  <th className="text-right px-4 py-3 text-gray-500 dark:text-muted font-semibold">Variación</th>
                  <th className="text-center px-4 py-3 text-gray-500 dark:text-muted font-semibold">Mapa calor</th>
                </tr>
              </thead>
              <tbody>
                {filteredProducts.map(p => (
                  <tr key={p.product} className="border-b border-gray-100 dark:border-card-border hover:bg-gray-50 dark:hover:bg-surface transition-colors">
                    <td className="px-4 py-3 font-semibold text-gray-900 dark:text-foreground">{p.product}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-muted">{p.category}</td>
      ...
      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5">
        <h3 className="font-extrabold text-amber-800 dark:text-amber-400 text-sm flex items-center gap-2"><Calendar className="h-4 w-4" /> Recomendaciones estacionales</h3>
