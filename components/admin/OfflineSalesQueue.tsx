    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className={cn(
            ...
            "bg-primary hover:bg-[#245a40] disabled:opacity-40"
          )}
        >
        ...
        {syncedCount > 0 && (
          <button
            onClick={clearSynced}
            className="flex items-center gap-2 rounded-lg border border-[var(--rule-base)] px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-gray-50 dark:border-gray-600 dark:text-[var(--text-tertiary)] dark:hover:bg-gray-800"
          >
        ...
        <button
          onClick={addDemoSale}
          className="ml-auto rounded-lg border border-dashed border-[var(--rule-base)] px-3 py-2 text-xs text-[var(--text-tertiary)] hover:border-gray-400 dark:border-gray-600"
        >
          + Venta demo
        </button>
      </div>
      ...
      {queue.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-[var(--rule-base)] py-10 dark:border-[var(--rule-base)]">
          <CheckCircle className="mb-2 h-8 w-8 text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]" />
          <p className="text-sm text-[var(--text-tertiary)]">Sin ventas en cola</p>
        </div>
      ) : (
            ...
            {(["pending", "syncing", "synced", "error"] as SyncStatus[]).map((s) => {
              ...
              return (
                <div key={s} className="flex items-center gap-1">
                  ...
                  <span className="text-[var(--text-tertiary)]">{cfg.label.toLowerCase()}</span>
                </div>
              );
            })}
          </div>
