  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-extrabold text-gray-900 dark:text-foreground flex items-center gap-2"><FileBarChart className="h-6 w-6 text-primary" /> Reportes Automáticos</h2>
          <p className="text-sm text-gray-500 dark:text-muted mt-0.5">Programa reportes que se generan y envían automáticamente</p>
        </div>
        <button onClick={openCreate} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold bg-primary text-white hover:bg-primary/90"><Plus className="h-4 w-4" /> Nuevo reporte</button>
      </div>
      ...
      <div className="space-y-3">
        {reports.map(r => (
          <div key={r.id} className={cn("bg-white dark:bg-card rounded-2xl border p-5 transition-opacity", r.active ? "border-gray-200 dark:border-card-border" : "border-gray-200 dark:border-card-border opacity-60")}>
