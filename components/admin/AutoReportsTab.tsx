            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span className="text-[length:var(--ts-2xs)] font-bold bg-gray-100 dark:bg-surface text-gray-500 dark:text-muted px-2 py-0.5 rounded-full">{SCHEDULE_LABELS[r.schedule]}</span>
                </div>
                <p className="text-xs text-gray-500 dark:text-muted mb-2">{r.description}</p>
                <div className="flex items-center gap-2 sm:gap-4 text-[length:var(--ts-2xs)] text-gray-400 flex-wrap">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-card rounded-xl p-3 sm:p-6 max-w-lg w-full mx-4 border border-[var(--rule-base)] dark:border-card-border" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div><label className="text-xs font-bold text-gray-500 dark:text-muted">Descripción</label><input value={formDesc} onChange={e => setFormDesc(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg border border-[var(--rule-base)] dark:border-card-border bg-white dark:bg-surface text-sm" /></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label className="text-xs font-bold text-gray-500 dark:text-muted">Formato</label><select value={formFormat} onChange={e => setFormFormat(e.target.value as Report["format"])} className="w-full mt-1 px-3 py-2 rounded-lg border border-[var(--rule-base)] dark:border-card-border bg-white dark:bg-surface text-sm"><option>PDF</option><option>Excel</option><option>CSV</option></select></div>
                <div><label className="text-xs font-bold text-gray-500 dark:text-muted">Frecuencia</label><select value={formSchedule} onChange={e => setFormSchedule(e.target.value as Report["schedule"])} className="w-full mt-1 px-3 py-2 rounded-lg border border-[var(--rule-base)] dark:border-card-border bg-white dark:bg-surface text-sm"><option value="diario">Diario</option><option value="semanal">Semanal</option><option value="mensual">Mensual</option><option value="manual">Manual</option></select></div>
              </div>
