  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <SectionTitle className="text-xl font-extrabold text-[var(--text-primary)] dark:text-foreground flex flex-wrap items-center gap-2"><Link className="h-6 w-6 text-primary" /> Webhooks & APIs</SectionTitle>
          ...
          <div key={wh.id} className={cn("bg-white dark:bg-card rounded-xl border overflow-hidden transition-opacity", wh.active ? "border-[var(--rule-base)] dark:border-card-border" : "border-[var(--rule-base)] dark:border-card-border opacity-60")}>
            <button onClick={() => setExpandedId(expandedId === wh.id ? null : wh.id)} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 dark:hover:bg-surface/50 transition-colors">
              <div className="flex flex-wrap items-center gap-3 text-left">
                ...
                <div>
                  <h4 className="font-bold text-sm text-[var(--text-primary)] dark:text-foreground">{wh.name}</h4>
        ...
        <div className="modal-backdrop p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-card rounded-xl p-3 sm:p-6 max-w-lg w-full mx-4 border border-[var(--rule-base)] dark:border-card-border max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div><label className="text-xs font-bold text-[var(--text-secondary)] dark:text-muted">URL</label><input value={formUrl} onChange={e => setFormUrl(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg border border-[var(--rule-base)] dark:border-card-border bg-white dark:bg-surface text-sm font-mono" placeholder="https://..." /></div>
              <div>
