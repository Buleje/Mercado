            ...
            <tbody>
              {filtered.map(t => (
                <tr key={t.id} className="border-t border-gray-100 dark:border-card-border hover:bg-gray-50 dark:hover:bg-accent/20 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs font-bold text-gray-700 dark:text-foreground">{t.number}<br/><span className="text-gray-400">{t.date}</span></td>
                  <td className="px-4 py-3 text-gray-700 dark:text-foreground">{t.clientName}<br/><span className="text-xs text-gray-400">{SOURCE_META[t.source]}</span></td>
                  <td className="px-4 py-3 font-bold text-gray-800 dark:text-foreground max-w-[200px] truncate">{t.subject}</td>
                  <td className="px-4 py-3"><span className={cn("text-xs font-bold px-2 py-0.5 rounded-full", PRIORITY_META[t.priority].bg, PRIORITY_META[t.priority].color)}>{PRIORITY_META[t.priority].label}</span></td>
                  <td className="px-4 py-3"><span className={cn("text-xs font-bold", STATUS_META[t.status].color)}>{STATUS_META[t.status].label}</span></td>
                  ...
                  <td className="px-4 py-3"><button onClick={() => setDetail(t)} className="text-primary hover:underline text-xs font-bold"><Eye className="h-3.5 w-3.5 inline" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
