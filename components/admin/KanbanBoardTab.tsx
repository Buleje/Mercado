...
const PRIORITY_CONFIG: Record<Priority, { label: string; color: string }> = {
  ...
  media: { label: "Media", color: "bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] dark:bg-primary/15 dark:text-[var(--data-success-500)]" },
...
const COL_CONFIG: Record<Column, { label: string; color: string; bg: string; icon: typeof Clock }> = {
  ...
  "en-progreso": { label: "En Progreso", color: "text-[var(--data-success-500)] dark:text-[var(--data-success-500)]", bg: "bg-primary/10 dark:bg-primary/15", icon: ListChecks },
  completado: { label: "Completado", color: "text-[var(--data-success-500)] dark:text-[var(--data-success-500)]", bg: "bg-primary/10 dark:bg-primary/15", icon: CheckCircle },
};
    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select value={filterAssignee} onChange={e => setFilterAssignee(e.target.value)} className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm font-semibold outline-none focus:border-primary">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
        {(["pendiente", "en-progreso", "completado"] as Column[]).map(col => {
          ...
          return (
            <div key={col} className={cn("rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4", c.bg)}>
      ...
      {showForm && (
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-3 sm:p-5 space-y-4">
          <CardTitle className="font-bold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Nueva Tarea</CardTitle>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4">
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Título de la tarea" className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm outline-none focus:border-primary" />
            ...
            <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Descripción" className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm outline-none focus:border-primary" />
            <div className="flex flex-wrap gap-3">
              <input type="date" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} className="flex-1 px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm outline-none focus:border-primary" />
              <select value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value as Priority })} className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm font-semibold outline-none focus:border-primary">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 sm:gap-4">
        {(["pendiente", "en-progreso", "completado"] as Column[]).map(col => {
          ...
          return (
            <div key={col} className={cn("rounded-xl border-2 border-dashed p-4 min-h-[300px]", conf.bg, "border-[var(--rule-base)] dark:border-[var(--rule-base)]")}
              ...
              <div className="space-y-3">
                {colTasks.map(t => (
                  <div key={t.id} draggable onDragStart={() => setDragTask(t.id)} className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4  hover:shadow-[var(--shadow-sm)] transition-shadow cursor-grab active:cursor-grabbing">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <GripVertical className="h-4 w-4 text-[var(--text-tertiary)] dark:text-[var(--text-secondary)] shrink-0" />
                        <h4 className="font-bold text-sm text-[var(--text-primary)] dark:text-[var(--text-primary)]">{t.title}</h4>
                      </div>
      ...
      {/* Edit modal */}
      {editing && (
        <div className="modal-backdrop p-4" onClick={() => setEditing(null)}>
          <div className="bg-[var(--surface-raised)] rounded-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="px-3 sm:px-6 py-4 border-b border-[var(--rule-soft)] dark:border-[var(--rule-base)] flex items-center justify-between">
              <CardTitle className="font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Editar Tarea</CardTitle>
            ...
            <div className="px-3 sm:px-6 py-5 space-y-3">
              <input value={editing.title} onChange={e => setEditing({ ...editing, title: e.target.value })} className="w-full px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm font-bold outline-none focus:border-primary" />
              <input value={editing.description} onChange={e => setEditing({ ...editing, description: e.target.value })} className="w-full px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm outline-none focus:border-primary" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input value={editing.assignee} onChange={e => setEditing({ ...editing, assignee: e.target.value })} className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm outline-none focus:border-primary" />
                <select value={editing.priority} onChange={e => setEditing({ ...editing, priority: e.target.value as Priority })} className="px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-[var(--surface-raised)] text-sm font-semibold outline-none focus:border-primary">
                  <option value="baja">Baja</option><option value="media">Media</option>
                  <option value="alta">Alta</option><option value="urgente">Urgente</option>
                </select>
              </div>
            </div>
            <div className="px-3 sm:px-6 py-4 border-t border-[var(--rule-soft)] dark:border-[var(--rule-base)] flex flex-wrap justify-end gap-2">
