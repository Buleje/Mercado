  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="relative max-w-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar proveedor, referencia..." className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 dark:border-card-border rounded-lg bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
      </div>
      ...
      {view === "mensajes" && (
        <div className="space-y-2">
          {filteredMessages.map(m => (
            <div key={m.id} className={cn("bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-lg p-4 flex items-start gap-3 cursor-pointer hover:shadow transition-shadow", !m.read && "border-l-4 border-l-primary")}>
