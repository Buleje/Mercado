  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <select value={filterPos} onChange={e => setFilterPos(e.target.value as Positioning | "todos")} className="text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-[var(--text-primary)] dark:text-foreground">
              ...
              {filtered.map(i => {
                ...
                return (
                  <tr key={i.id} className="hover:bg-gray-50/50 dark:hover:bg-surface/30 transition-colors">
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3 text-right font-bold text-xs text-[var(--text-primary)] dark:text-foreground">{fmt(i.ownPrice)}</td>
      ...
      <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-3 sm:p-5 space-y-3">
        <CardTitle className="font-extrabold text-sm text-[var(--text-primary)] dark:text-foreground flex flex-wrap items-center gap-2">
        ...
        <div className="modal-backdrop p-4" onClick={() => setDetail(null)}>
          <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-3 sm:p-6 w-full max-w-md space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[var(--text-secondary)]">Nuestro precio</span><span className="font-bold text-[var(--text-primary)] dark:text-foreground">{fmt(detail.ownPrice)}</span></div>
