  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-extrabold text-gray-900 dark:text-foreground flex flex-wrap items-center gap-2"><ShoppingCart className="h-6 w-6 text-primary" /> Análisis de Cesta</h2>
      ...
      <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border p-3 sm:p-5">
        <h3 className="font-bold text-sm text-gray-900 dark:text-foreground mb-3 flex flex-wrap items-center gap-2"><Package className="h-4 w-4 text-primary" /> Combos más frecuentes</h3>
      ...
      <div className="bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-card-border overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-5 py-3 bg-gray-50 dark:bg-surface gap-2">
          <h3 className="font-bold text-sm text-gray-700 dark:text-foreground flex flex-wrap items-center gap-2"><Link2 className="h-4 w-4 text-primary" /> Reglas de asociación</h3>
          ...
          <table className="w-full min-w-[600px] text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-500 dark:text-muted bg-gray-50 dark:bg-surface">
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold">Producto A</th>
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold" />
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold">Producto B</th>
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold text-right">Soporte</th>
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold text-right">Confianza</th>
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold text-right">Lift</th>
                <th className="px-2 sm:px-4 py-1.5 sm:py-2 font-bold text-right">Transacciones</th>
              </tr>
            </thead>
            <tbody>
              {sorted.map((a, i) => (
                <tr key={i} className="border-t border-gray-100 dark:border-card-border hover:bg-gray-50 dark:hover:bg-surface/50">
                  <td className="px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-900 dark:text-foreground">{a.productA}</td>
                  <td className="px-2 py-3"><ArrowRight className="h-3 w-3 text-gray-400" /></td>
                  <td className="px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-900 dark:text-foreground">{a.productB}</td>
                  <td className="px-2 sm:px-4 py-2 sm:py-3 text-right"><span className="text-xs font-bold text-blue-500">{(a.support * 100).toFixed(0)}%</span></td>
                  ...
                  <td className="px-2 sm:px-4 py-2 sm:py-3 text-right"><span className={cn("text-xs font-bold", a.lift >= 2 ? "text-violet-500" : "text-gray-500")}>{a.lift.toFixed(1)}x</span></td>
                  <td className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs font-bold text-gray-700 dark:text-foreground">{a.count}</td>
                </tr>
              ))}
            </tbody>
          </table>
