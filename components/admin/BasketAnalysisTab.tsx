            ...
            {crossSell.map((a, i) => (
              <div key={i} className="bg-white dark:bg-card rounded-xl p-3 flex items-center gap-3 border border-amber-100 dark:border-amber-900/20">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[length:var(--ts-2xs)] text-gray-400">{(a.confidence * 100).toFixed(0)}% de las veces · {a.count} transacciones</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {topCombos.map((a, i) => (
              <div key={i} className="bg-gray-50 dark:bg-surface rounded-xl p-4">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[length:var(--ts-2xs)] text-gray-400">{a.count}x · {a.category}</span>
                </div>
                <div className="flex flex-wrap gap-1 mb-2">
                  <span className="text-xs bg-white dark:bg-card px-2 py-1 rounded-lg border border-[var(--rule-base)] dark:border-card-border font-semibold text-gray-700 dark:text-foreground">{a.productA}</span>
                  <ArrowRight className="h-3.5 w-3.5 text-gray-400 self-center" />
                  <span className="text-xs bg-white dark:bg-card px-2 py-1 rounded-lg border border-[var(--rule-base)] dark:border-card-border font-semibold text-gray-700 dark:text-foreground">{a.productB}</span>
                </div>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[length:var(--ts-2xs)] font-bold text-emerald-600 dark:text-emerald-400">{(a.confidence * 100).toFixed(0)}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-5 py-3 bg-gray-50 dark:bg-surface gap-2 flex-wrap">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)}
              className="px-2 py-0.5 rounded text-[length:var(--ts-2xs)] font-bold bg-gray-200 dark:bg-card text-gray-600 dark:text-muted border-0">
              {categories.map(c => <option key={c} value={c}>{c === "all" ? "Todas las categorías" : c}</option>)}
            </select>
            {/* Ordenar */}
            <div className="flex items-center gap-1">
              <span className="text-[length:var(--ts-2xs)] text-gray-400">Orden:</span>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] text-sm">
              ...
              <tbody>
                {sorted.map((a, i) => (
                  <tr key={i} className="border-t border-[var(--rule-soft)] dark:border-card-border hover:bg-gray-50 dark:hover:bg-surface/50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className="text-[length:var(--ts-2xs)] font-bold px-2 py-0.5 rounded-full bg-gray-100 dark:bg-surface text-gray-600 dark:text-muted">{a.category}</span>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <span className={cn("text-xs font-bold", a.lift >= 2 ? "text-[var(--text-secondary)]" : a.lift >= 1.5 ? "text-amber-500" : "text-gray-500")}>{a.lift.toFixed(1)}x</span>
                    </td>
