    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["todos", "activos", "inactivos"] as const).map(f => (
          <button key={f} onClick={() => setShowActive(f)} className={cn("px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-lg text-sm font-bold transition-colors", showActive === f ? "bg-teal-500 text-white" : "bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-accent")}>
          ...
          return (
            <div key={kit.id} className={cn("bg-white dark:bg-card rounded-xl border border-[var(--rule-base)] dark:border-card-border p-3 sm:p-5 hover:shadow-sm transition-shadow", !kit.active && "opacity-60")}>
              ...
              <div className="mt-3 pt-3 border-t border-[var(--rule-soft)] dark:border-card-border grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-center">
                <div><p className="text-[length:var(--ts-2xs)] text-gray-400">Costo</p><p className="text-sm font-bold text-gray-600">{fmt(cost)}</p></div>
                <div><p className="text-[length:var(--ts-2xs)] text-gray-400">Precio Venta</p><p className="text-sm font-extrabold text-gray-900 dark:text-foreground">{fmt(kit.salePrice)}</p></div>
                <div><p className="text-[length:var(--ts-2xs)] text-gray-400">Margen</p><p className={cn("text-sm font-extrabold", margin >= 20 ? "text-emerald-600" : margin >= 10 ? "text-amber-600" : "text-red-600")}>{margin.toFixed(1)}%</p></div>
              </div>
