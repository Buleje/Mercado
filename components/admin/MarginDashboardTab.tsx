    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-foreground">{fmt(data.summary.totalRevenue)}</p>
        ...
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-foreground">{fmt(data.summary.totalGrossProfit)}</p>
        ...
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
          ...
          <p className={cn("text-xl sm:text-2xl font-extrabold", data.summary.overallMarginPct >= 30 ? "text-emerald-600" : data.summary.overallMarginPct >= 15 ? "text-amber-600" : "text-red-500")}>{data.summary.overallMarginPct.toFixed(1)}%</p>
        ...
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-foreground">{data.summary.productsWithoutCost}</p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {view === "products" && (
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar..." className="ml-auto px-2 sm:px-4 py-1.5 sm:py-2 border border-gray-200 dark:border-card-border rounded-xl bg-white dark:bg-surface text-sm w-48" />
        )}
      </div>
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-card-border text-left">
                  ...
                  <th className="px-2 sm:px-4 py-2 sm:py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase tracking-wide text-right hidden md:table-cell">Costo</th>
                  <th className="px-2 sm:px-4 py-2 sm:py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase tracking-wide text-right hidden md:table-cell">Ganancia</th>
                  <th className="px-2 sm:px-4 py-2 sm:py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase tracking-wide text-right">Margen</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {filtered.map(p => (
                  <tr key={p.productId} className="hover:bg-gray-50 dark:hover:bg-surface transition-colors">
                    ...
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs text-gray-500 dark:text-muted hidden sm:table-cell">{p.category}</td>
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-right text-gray-700 dark:text-foreground">{fmt(p.revenue)}</td>
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-right text-gray-500 dark:text-muted hidden md:table-cell">{p.cost > 0 ? fmt(p.cost) : <span className="text-amber-400">—</span>}</td>
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-right text-gray-700 dark:text-foreground hidden md:table-cell">{fmt(p.grossProfit)}</td>
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-right">{pct(p.marginPct, true)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
          {data.byCategory.map(cat => (
            <div key={cat.category} className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-center">
