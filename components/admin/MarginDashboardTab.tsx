...
const pctColor = (n: number) =>
  n >= 30 ? "text-emerald-600" : n >= 15 ? "text-amber-600" : "text-red-500";
...
export default function MarginDashboardTab() {
  ...
  if (!data) return (
    <div className="text-center py-20 text-gray-400 dark:text-muted">Error cargando datos de márgenes</div>
  );
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      {/* Alerta de umbral */}
      <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-sm font-bold text-amber-600 w-10">{threshold}%</span>
        </div>
        {belowThreshold.length > 0 ? (
          ...
          <span className="ml-auto text-sm font-bold text-emerald-600">Todos sobre el umbral</span>
        )}
      </div>
          ...
          <button
            ...
            className={cn(
              ...
              view === v
                ...
                : "bg-gray-100 dark:bg-surface text-gray-600 dark:text-muted hover:bg-gray-200 dark:hover:bg-card"
            )}
          >
        ...
        {view === "products" && (
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 rounded-xl border border-gray-200 dark:border-card-border text-sm text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-surface"
            >
      ...
      {view === "products" && (
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl overflow-hidden shadow-sm">
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-card-border text-left">
                  ...
                  <th className="px-4 py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase text-right hidden md:table-cell">Costo</th>
                  <th className="px-4 py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase text-right hidden md:table-cell">Ganancia</th>
                  <th className="px-4 py-3 text-xs font-bold text-gray-500 dark:text-muted uppercase text-right">Margen</th>
                </tr>
              </thead>
                ...
                {filtered.map((p) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      p.marginPct < threshold && p.revenue > 0 && "bg-red-50/40 dark:bg-red-950/10"
                    )}
                  >
                    ...
                    <td className="px-4 py-2.5 text-right text-gray-500 dark:text-muted hidden md:table-cell">
                      {p.cost > 0 ? fmt(p.cost) : <span className="text-amber-400">—</span>}
                    </td>
          ...
          {filtered.length === 0 && (
            <div className="h-20 flex items-center justify-center text-gray-400 dark:text-muted text-sm">
          ...
          {data.byCategory.map((cat) => (
            <div key={cat.category} className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                <div>
                  <p className="text-sm font-bold text-gray-900 dark:text-foreground">{fmt(cat.revenue)}</p>
                  <p className="text-xs text-gray-400 dark:text-muted">Ingresos</p>
                </div>
                <div>
                  <p className="text-sm font-bold text-gray-900 dark:text-foreground">{fmt(cat.cost)}</p>
                  <p className="text-xs text-gray-400 dark:text-muted">Costo</p>
                </div>
                <div>
                  <p className="text-sm font-bold text-emerald-600">{fmt(cat.grossProfit)}</p>
                  <p className="text-xs text-gray-400 dark:text-muted">Ganancia</p>
                </div>
              </div>
            </div>
          ))}
            ...
            <div className="divide-y divide-gray-50 dark:divide-card-border">
              {top10Best.map((p, i) => (
                <div key={p.productId} className="px-4 py-3 flex items-center gap-3">
                  <span className="text-xs font-extrabold text-gray-300 dark:text-muted w-5">{i + 1}</span>
              ...
              {top10Best.length === 0 && (
                <p className="px-4 py-8 text-center text-sm text-gray-400 dark:text-muted">Sin datos</p>
              )}
            </div>
          </div>
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/40 rounded-2xl p-4">
          <div className="flex items-start gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-600/80">Revisa precios de venta o negocia mejor con proveedores</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {belowThreshold.slice(0, 8).map((p) => (
              <span
                key={p.productId}
                className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white dark:bg-card border border-red-200 dark:border-red-800 text-xs text-red-700 dark:text-red-400 font-semibold"
              >
                {p.name}
            ...
            {belowThreshold.length > 8 && (
              <span className="text-xs text-red-500 self-center">+{belowThreshold.length - 8} más</span>
            )}
          </div>
        </div>
      )}
