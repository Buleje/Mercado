...
export default function NotasCreditoModule() {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      {showShortcuts && (
        <div className="bg-white border border-[var(--rule-base)] rounded-xl p-3 text-xs text-[var(--text-secondary)]">
            ...
            <div className="flex items-center gap-0.5 bg-gray-100 rounded-xl p-1">
              {([["table", LayoutList], ["cards", LayoutGrid], ["kanban", Kanban]] as const).map(([mode, Icon]) => (
                <button key={mode} onClick={() => setViewMode(mode)}
                  ...
                  className={cn("p-1.5 rounded-lg transition-colors", viewMode === mode ? "bg-white text-primary " : "text-[var(--text-secondary)] hover:text-[var(--text-primary)]")}>
                  <Icon className="h-3.5 w-3.5" />
                </button>
              ))}
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-gray-50 rounded-xl p-3">
                <div>
                  ...
                  <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                    className="w-full px-2 py-1.5 rounded-lg border border-[var(--rule-base)] bg-white text-xs text-[var(--text-primary)]" />
                </div>
                <div>
        ...
        {someChecked && (
          <m.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
            <div className="flex items-center gap-3 px-4 py-3 bg-primary/5 border border-primary/20 rounded-xl">
              ...
              <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-white border border-[var(--rule-base)] text-[var(--text-primary)] hover:bg-gray-50 transition-colors">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" value={pickerSearch} onChange={e => setPickerSearch(e.target.value)}
                          ...
                          className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-[var(--rule-base)] bg-white text-sm text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-primary/30" />
                        ...
                        <div className={cn("rounded-xl p-4 border-2", (DOC_STYLE[selectedVenta.comprobanteTipo] || DOC_STYLE.ticket).border, (DOC_STYLE[selectedVenta.comprobanteTipo] || DOC_STYLE.ticket).bg)}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <span className="font-mono text-sm font-bold text-[var(--text-primary)] ml-2">{selectedVenta.comprobanteNumero || `#${selectedVenta.numero}`}</span>
                            </div>
                            <div className="flex-1" />
