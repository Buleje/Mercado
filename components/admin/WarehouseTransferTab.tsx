    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(["todas", "pendiente", "en-transito", "recibido", "cancelado"] as const).map((status) => (
          <button key={status} onClick={() => setStatusFilter(status)} className={cn("rounded-lg px-2 sm:px-4 py-1.5 sm:py-2.5 text-sm font-bold transition-colors", statusFilter === status ? "bg-primary text-white" : "border border-[var(--rule-base)] bg-white text-gray-600 dark:border-card-border dark:bg-card dark:text-muted")}>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap items-center gap-2">
                      {(["pendiente", "en-transito", "recibido", "cancelado"] as TransferStatus[]).map((status) => (
                        <button key={status} onClick={() => updateStatus(transfer.id, status)} className="rounded-lg border border-[var(--rule-base)] px-2 py-1 text-[length:var(--ts-xs)] font-bold text-gray-600 hover:bg-gray-50 dark:border-card-border dark:text-muted dark:hover:bg-accent">
