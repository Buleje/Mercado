...
const TYPE_CONFIG: Record<ReminderType, { label: string; icon: typeof Bell; color: string }> = {
  pago:        { label: "Pago", icon: DollarSign, color: "text-[var(--data-success-500)]" },
  inventario:  { label: "Inventario", icon: Package, color: "text-[var(--data-success-500)]" },
  tarea:       { label: "Tarea", icon: CheckCircle, color: "text-[var(--text-secondary)]" },
  vencimiento: { label: "Vencimiento", icon: AlertTriangle, color: "text-[var(--data-error-500)]" },
  cliente:     { label: "Cliente", icon: Bell, color: "text-[var(--data-warning-500)]" },
...
const PRIORITY_CONFIG: Record<ReminderPriority, { label: string; color: string; dot: string }> = {
  alta:  { label: "Alta",  color: "bg-[var(--data-error-100)] dark:bg-[var(--data-error-500)]/30 text-[var(--data-error-500)] dark:text-[var(--data-error-500)]",    dot: "bg-[var(--data-error-500)]" },
  media: { label: "Media", color: "bg-[var(--data-warning-100)] dark:bg-[var(--data-warning-500)]/30 text-[var(--data-warning-500)] dark:text-[var(--data-warning-500)]", dot: "bg-[var(--data-warning-500)]" },
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
        <div>
          <SectionTitle className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Recordatorios Inteligentes</SectionTitle>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
          ...
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--data-warning-500)] mt-1">{stats.pending}</p>
        </div>
        <div className="bg-[var(--data-error-50)] dark:bg-red-950/20 rounded-xl border border-[var(--data-error-500)] dark:border-[var(--data-error-500)] p-4 ">
          <p className="text-xs text-[var(--data-error-500)] dark:text-[var(--data-error-500)] font-semibold">Prioridad alta</p>
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--data-error-500)] dark:text-[var(--data-error-500)] mt-1">{stats.highPriority}</p>
        </div>
        <div className="bg-primary/10 dark:bg-primary/15 rounded-xl border border-[var(--data-success-500)]/30 dark:border-[var(--data-success-500)]/30 p-4 ">
          <p className="text-xs text-[var(--data-success-500)] dark:text-[var(--data-success-500)] font-semibold">Completados</p>
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--data-success-500)] dark:text-[var(--data-success-500)] mt-1">{stats.completed}</p>
        </div>
        <div className="bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4 ">
          <p className="text-xs text-[var(--text-secondary)] dark:text-muted font-semibold">Auto-generados</p>
          <p className="text-xl sm:text-2xl font-extrabold text-[var(--data-success-500)] mt-1">{stats.autoGenerated}</p>
        </div>
      </div>

      {/* Filters */}
      <div className="flex flex-wrap items-center gap-3">
        <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as typeof typeFilter)} className="px-3 py-2.5 rounded-lg border-2 border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-sm outline-none focus:border-primary">
      ...
      <div className="space-y-3">
        {filtered.map(r => {
          ...
          return (
            <div key={r.id} className={cn("bg-[var(--surface-raised)] rounded-xl border border-[var(--rule-base)] dark:border-[var(--rule-base)] p-4  flex items-start gap-2 sm:gap-4 transition-all", isCompleted && "opacity-60")}>
              <button onClick={() => handleComplete(r.id)} className={cn("mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors", isCompleted ? "bg-primary/10 border-[var(--data-success-500)]/30" : "border-[var(--rule-base)] dark:border-[var(--rule-base)] hover:border-primary")}>
                {isCompleted && <Check className="h-3.5 w-3.5 text-white" />}
              </button>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <h3 className={cn("font-bold text-sm", isCompleted ? "line-through text-[var(--text-tertiary)] dark:text-muted" : "text-[var(--text-primary)] dark:text-[var(--text-primary)]")}>{r.title}</h3>
                  {r.autoGenerated && <span className="px-1.5 py-0.5 rounded bg-primary/10 dark:bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] dark:text-[var(--data-success-500)] text-[length:var(--ts-2xs)] font-bold">AUTO</span>}
                </div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className={cn("text-xs font-semibold", r.dueDate < new Date().toISOString().slice(0, 10) && r.status !== "completado" ? "text-[var(--data-error-500)]" : "text-[var(--text-tertiary)] dark:text-muted")}>{getDaysText(r.dueDate)}</span>
                </div>
              </div>
              <button onClick={() => handleDelete(r.id)} className="p-1.5 rounded-lg text-[var(--text-tertiary)] hover:text-[var(--data-error-500)] hover:bg-[var(--data-error-50)] dark:hover:bg-red-950/30 transition-colors shrink-0">
        ...
        <div className="modal-backdrop p-4" onClick={() => setShowNew(false)}>
          <div className="bg-[var(--surface-raised)] rounded-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-3 sm:px-6 py-4 border-b border-[var(--rule-soft)] dark:border-[var(--rule-base)]">
              <CardTitle className="font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">Nuevo recordatorio</CardTitle>
