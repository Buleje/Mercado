        ...
        {filtered.map(r => {
          ...
          return (
            <div key={r.id} className={cn("bg-white dark:bg-card rounded-xl border border-[var(--rule-base)] dark:border-card-border p-4  flex items-start gap-2 sm:gap-4 transition-all", isCompleted && "opacity-60")}>
              <button onClick={() => handleComplete(r.id)} className={cn("mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors", isCompleted ? "bg-[var(--accent-soft)] border-[var(--data-success-500)]/30" : "border-[var(--rule-base)] dark:border-card-border hover:border-primary")}>
