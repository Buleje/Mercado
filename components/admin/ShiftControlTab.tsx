...
function ModuleTooltip() {
  ...
  return (
    <div className="relative inline-block">
      <button type="button" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} onFocus={() => setOpen(true)} onBlur={() => setOpen(false)} className="text-[var(--text-tertiary)] hover:text-primary transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface-canvas)]" aria-label="Ayuda">
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {onNavigateToArqueo && (
            <AdminTooltip content="Ir al módulo de arqueo de caja detallado">
              <button onClick={onNavigateToArqueo} aria-label="Ver Arqueo de Caja" className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-primary/30 bg-primary/5 text-[var(--accent-ink)] dark:text-[var(--accent)] text-sm font-semibold hover:bg-primary/10 transition-colors">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        {[
          { label: "Ventas hoy", value: fmt(stats.totalSalesToday), icon: BarChart2, color: "text-[var(--data-success-500)]", bg: "bg-primary/10 dark:bg-primary/15" },
          { label: "Pedidos hoy", value: String(stats.totalOrdersToday), icon: CheckCircle, color: "text-[var(--data-success-500)]", bg: "bg-primary/10 dark:bg-primary/15" },
          { label: "Turnos activos", value: String(stats.activeCount), icon: Timer, color: "text-[var(--data-warning-500)]", bg: "bg-[var(--data-warning-50)] dark:bg-amber-950/30" },
          { label: "Diferencia en caja", value: fmt(Math.abs(stats.totalDifference)), icon: stats.totalDifference === 0 ? CheckCircle : AlertTriangle, color: stats.totalDifference === 0 ? "text-[var(--data-success-500)]" : stats.totalDifference > 0 ? "text-[var(--data-success-500)]" : "text-[var(--data-error-500)]", bg: stats.totalDifference === 0 ? "bg-primary/10 dark:bg-primary/15" : "bg-[var(--data-error-50)] dark:bg-red-950/30" },
      ...
      {showOpenForm && (
        <div className="bg-[var(--surface-raised)] border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-xl p-3 sm:p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            ...
            <button onClick={handleOpenShift} className="px-2 sm:px-4 py-1.5 sm:py-2 text-sm rounded-lg bg-primary/10 text-white font-semibold hover:bg-primary/10">Abrir turno</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-[var(--surface-raised)] border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-xl p-3 sm:p-6 w-full max-w-md space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              {closeForm.closingCash && (
                <div className={cn("flex items-center justify-between rounded-xl px-2 sm:px-4 py-2 sm:py-3 text-sm font-bold", parseFloat(closeForm.closingCash) === showCloseForm.expectedCash ? "bg-primary/10 dark:bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] dark:text-[var(--data-success-500)]" : parseFloat(closeForm.closingCash) > showCloseForm.expectedCash ? "bg-primary/10 dark:bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] dark:text-[var(--data-success-500)]" : "bg-[var(--data-error-50)] dark:bg-red-950/20 text-[var(--data-error-500)] dark:text-[var(--data-error-500)]")}>
            ...
            <div key={i} className="bg-[var(--surface-raised)] border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-xl p-3 sm:p-5 animate-pulse">
              <div className="flex flex-wrap items-center gap-3 mb-4">
                ...
                <div className="flex-1 space-y-2">
                  <div className="h-4 bg-[var(--rule-soft)] dark:bg-surface rounded w-32" />
                  <div className="h-3 bg-[var(--surface-sunken)] dark:bg-surface/50 rounded w-48" />
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {[1,2,3,4].map(j => <div key={j} className="h-12 rounded-xl bg-[var(--surface-sunken)] dark:bg-surface/50" />)}
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={sh.id} className={cn("bg-[var(--surface-raised)] border rounded-xl p-3 sm:p-5 transition-all", isOpen ? "border-[var(--data-success-500)]/30 dark:border-[var(--data-success-500)]/30 " : "border-[var(--rule-base)] dark:border-[var(--rule-base)]")}>
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div className="flex flex-wrap items-center gap-3">
                  <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-sm shrink-0", isOpen ? "bg-primary/10" : "bg-[var(--rule-mid)] dark:bg-surface")}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className={cn("text-xs font-bold px-2.5 py-1 rounded-full", isOpen ? "bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] dark:bg-primary/15 dark:text-[var(--data-success-500)]" : "bg-[var(--surface-sunken)] text-[var(--text-secondary)] dark:bg-surface dark:text-muted")}>
                    {isOpen ? "Activo" : sh.status === "revisando" ? "Revisando" : `Cerrado — ${duration(sh.openedAt, sh.closedAt)}`}
                  </span>
                  {isOpen && (
                    <button onClick={() => { setShowCloseForm(sh); setCloseForm({ closingCash: "", notes: "" }); }} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[var(--data-error-50)] dark:bg-red-950/20 text-[var(--data-error-500)] text-xs font-semibold hover:bg-[var(--data-error-100)] dark:hover:bg-[var(--data-error-500)]/30 transition-colors">
                      <Square className="h-3.5 w-3.5" /> Cerrar
                    </button>
                  )}
                  {!isOpen && (
                    <button onClick={() => setPrintingShift(sh)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary/10 dark:bg-[var(--data-success-500)]/12 text-[var(--data-success-700)] dark:text-[var(--data-success-500)] text-xs font-semibold hover:bg-primary/10 dark:hover:bg-primary/15 transition-colors">
