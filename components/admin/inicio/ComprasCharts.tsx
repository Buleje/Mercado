...
import { BarChart3, Truck, AlertTriangle, Clock, CreditCard, DollarSign } from "@buleje/design-system/icons";
    ...
    <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl px-3 py-2 text-xs">
      ...
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <span className="font-bold text-[var(--text-primary)] dark:text-foreground">{prefix}{typeof p.value === "number" ? `S/ ${p.value.toFixed(2)}` : p.value}</span>
        </div>
      ))}
    </div>
  );
              ...
              {data.cuentasPorVencer.map((c, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 border-b border-gray-50 dark:border-[var(--rule-base)] last:border-0">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-xs font-bold text-[var(--text-primary)] dark:text-foreground">S/ {c.monto.toFixed(2)}</span>
                    <span className={cn(
                      "text-[length:var(--ts-2xs)] font-bold px-1.5 py-0.5 rounded whitespace-nowrap",
                      c.status === "vencido" ? "bg-[var(--data-error-50)] text-[var(--data-error)] dark:bg-red-950/30 dark:text-[var(--data-error)]"
                        : c.status === "urgente" ? "bg-[var(--data-warning-50)] text-[var(--data-warning)] dark:bg-amber-950/30 dark:text-[var(--data-warning)]"
              ...
              {data.topProveedores.map((p, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[length:var(--ts-2xs)] text-[var(--text-tertiary)]">{p.ordenes} órdenes</p>
                  </div>
                  <span className="text-xs font-bold text-[var(--text-primary)] dark:text-foreground shrink-0">S/ {p.total.toFixed(2)}</span>
                </div>
              ))}
            </div>
