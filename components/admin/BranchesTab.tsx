...
const TRANSFER_STATUS: Record<Transfer["status"], { label: string; color: string; bg: string }> = {
  ...
  "en-transito": { label: "En Tránsito", color: "text-[var(--data-success)]",    bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Sucursales activas", value: String(stats.active), color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
          { label: "Venta mensual total", value: fmt(stats.totalRevenue), color: "text-[var(--data-success)]", bg: "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)]" },
          ...
          { label: "Transferencias activas", value: String(stats.pendingTransfers), color: "text-[var(--data-warning)]", bg: "bg-[var(--data-warning-50)] dark:bg-amber-950/30" },
      ...
      {view === "branches" && (
        <>
          <div className="relative max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar sucursal..." className="w-full pl-9 pr-3 py-2 text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg bg-white dark:bg-surface text-[var(--text-primary)] dark:text-foreground" />
          </div>
            ...
            {filteredBranches.map(b => (
              <div key={b.id} className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-4 space-y-3 hover:shadow-lg transition-shadow">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <span className={cn("flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full", b.status === "activa" ? "bg-[var(--accent-soft)] dark:bg-[var(--accent-muted)] text-[var(--data-success)]" : "bg-[var(--data-error-100)] dark:bg-[var(--data-error)]/30 text-[var(--data-error)]")}>
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-[var(--data-success)]">{fmt(b.monthlyRevenue)}</span>
                </div>
                {b.status === "activa" && (
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-[var(--text-secondary)] dark:text-muted">SKUs en stock</span>
                    <span className="font-bold text-[var(--text-primary)] dark:text-foreground">{b.inventory.length} productos</span>
                  </div>
                )}
                ...
                {transfers.map(t => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-gray-50/50 dark:hover:bg-surface/30 transition-colors">
                      ...
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs font-bold text-[var(--text-primary)] dark:text-foreground">{t.qty} {t.unit}</td>
      ...
      {view === "branches" && (
        <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-3 sm:p-5 space-y-3">
          <CardTitle className="font-extrabold text-sm text-[var(--text-primary)] dark:text-foreground flex flex-wrap items-center gap-2"><BarChart3 className="h-4 w-4 text-primary" /> Comparativa de ventas mensuales</CardTitle>
