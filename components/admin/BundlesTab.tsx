...
type Product = { id: string; name: string; price: number; image: string; category: string; unit: string };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-xl font-extrabold text-gray-900 dark:text-foreground flex items-center gap-2"><Package className="h-6 w-6 text-primary" />Combos y Paquetes</h2>
        <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary/90 transition flex items-center gap-2"><Plus className="h-4 w-4" />Nuevo Combo</button>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4 text-center">
          <p className="text-2xl font-extrabold text-gray-900 dark:text-foreground">{bundles.length}</p>
          <p className="text-xs text-gray-400">Total Combos</p>
        </div>
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4 text-center">
          <p className="text-2xl font-extrabold text-emerald-600">{bundles.filter(b => b.active).length}</p>
          <p className="text-xs text-gray-400">Activos</p>
        </div>
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl p-4 text-center">
          <p className="text-2xl font-extrabold text-gray-400">{bundles.filter(b => !b.active).length}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => { setShowForm(false); resetForm(); }}>
          <div className="bg-white dark:bg-card rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4" onClick={e => e.stopPropagation()}>
              ...
              {items.length > 0 && (
                <div className="space-y-2">
                  {items.map(i => {
                    ...
                    if (!p) return null;
                    return (
                      <div key={i.productId} className="flex items-center gap-2 bg-gray-50 dark:bg-surface rounded-lg px-3 py-2">
            ...
            <button onClick={save} disabled={saving || !name || !price || items.length === 0} className="w-full py-2.5 bg-primary text-white rounded-xl font-bold text-sm hover:bg-primary/90 transition disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              <div key={b.id} className={cn("bg-white dark:bg-card border rounded-2xl overflow-hidden", b.active ? "border-gray-200 dark:border-card-border" : "border-gray-100 dark:border-card-border opacity-60")}>
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex items-end justify-between pt-2 border-t border-gray-100 dark:border-card-border">
                    <div>
                      <p className="text-2xl font-extrabold text-primary">S/{b.price.toFixed(2)}</p>
