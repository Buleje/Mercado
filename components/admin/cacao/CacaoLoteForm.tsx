...
import { Leaf, Loader2, X, Search, Check, Coins, Beaker, Users, Droplets } from "@buleje/design-system/icons";
import { CardTitle } from "@buleje/design-system";
import AdminModal from "@/components/admin/shared/AdminModal";
import { csrfHeaders } from "@/lib/csrf-client";
import {
  CACAO_VARIEDADES, cacaoGrade, cacaoFermentationIndex, cacaoLiquidacion,
...
export default function CacaoLoteForm({ onClose, onSaved }: Props) {
  ...
  const cutSum = (Number(pctBienFermentado) || 0) + (Number(pctVioleta) || 0) + (Number(pctPizarroso) || 0) + (Number(pctMohoso) || 0);
      ...
      <div className="flex h-full max-h-[90vh] flex-col bg-[var(--surface-raised)]">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--rule-base)] px-5 py-4">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--accent-soft)] text-[var(--accent)]"><Leaf className="h-5 w-5" strokeWidth={1.75} /></span>
            <div><CardTitle as="h2" className="text-base font-bold text-[var(--text-primary)]">Nuevo lote de acopio</CardTitle><p className="text-xs text-[var(--text-tertiary)]">Cacao · grado y pago se calculan al vuelo (NTP-ISO)</p></div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_300px]">
            ...
            <form id="cacao-lote-form" onSubmit={submit} className="space-y-5 px-5 py-5">
              ...
              <Section icon={Leaf} title="Datos del lote">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Tipo de grano">
                    <Segmented value={tipoGrano} onChange={setTipoGrano} options={[{ v: "seco", label: "Seco" }, { v: "humedo", label: "Húmedo" }]} />
                  </Field>
                </div>

                {/* Picker de productor */}
                <div className="space-y-2 rounded-xl border border-[var(--accent)]/30 bg-[var(--accent-soft)]/40 p-3">
                  <span className="flex items-center gap-1.5 text-[length:var(--ts-2xs)] font-bold uppercase tracking-[var(--ls-wider)] text-[var(--accent)]"><Users className="h-3.5 w-3.5" />Productor</span>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <Field label="Humedad (%)" hint={humOk === false ? "⚠ sobre 7%" : "meta ≤ 7%"}><input type="number" step="0.1" value={humedadPct} onChange={(e) => setHumedadPct(e.target.value)} placeholder="7.0" className={`${I} font-mono tabular-nums ${humOk === false ? "border-[var(--data-warning-400)]" : humOk === true ? "border-[var(--data-success-400)]" : ""}`} /></Field>
                </div>
              </Section>

              <Section icon={Beaker} title="Prueba de corte" hint="NTP-ISO 1114 · % de granos">
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <Field label="Bien ferm." hint="marrón"><input type="number" step="1" value={pctBienFermentado} onChange={(e) => setBien(e.target.value)} placeholder="%" className={`${I} font-mono tabular-nums`} /></Field>
                  <Field label="Violeta" hint="parcial"><input type="number" step="1" value={pctVioleta} onChange={(e) => setVioleta(e.target.value)} placeholder="%" className={`${I} font-mono tabular-nums`} /></Field>
                  <Field label="Pizarroso" hint="sin ferm."><input type="number" step="1" value={pctPizarroso} onChange={(e) => setPizarroso(e.target.value)} placeholder="%" className={`${I} font-mono tabular-nums`} /></Field>
                  <Field label="Mohoso" hint="defecto"><input type="number" step="1" value={pctMohoso} onChange={(e) => setMohoso(e.target.value)} placeholder="%" className={`${I} font-mono tabular-nums`} /></Field>
                </div>
                {hasCut && (
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-[var(--text-tertiary)]">Suma de la prueba</span>
                    <span className={`font-mono font-bold tabular-nums ${cutSum > 100 ? "text-[var(--data-error-700)]" : cutSum >= 95 && cutSum <= 100 ? "text-[var(--data-success-700)]" : "text-[var(--data-warning-700)]"}`}>{cutSum}% {cutSum > 100 ? "· revisá, pasa de 100" : cutSum < 95 ? "· debería sumar ~100" : "· ok"}</span>
                  </div>
                )}
                <div className="w-40"><Field label="Granos evaluados"><input type="number" value={cutGranos} onChange={(e) => setCutGranos(e.target.value)} placeholder="100" className={`${I} h-9 font-mono tabular-nums`} /></Field></div>
              </Section>
              ...
              <Section icon={Coins} title="Liquidación al productor">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Premio S//kg" hint="fino de aroma"><input type="number" step="0.01" value={premioPorKg} onChange={(e) => setPremioPorKg(e.target.value)} placeholder="0.50" className={`${I} font-mono tabular-nums`} /></Field>
                </div>
            ...
            <aside className="border-t-2 border-[var(--rule-soft)] bg-[var(--surface-canvas)]/40 px-5 py-5 lg:sticky lg:top-0 lg:self-start lg:border-l-2 lg:border-t-0">
              <p className="mb-3 text-[length:var(--ts-2xs)] font-bold uppercase tracking-[var(--ls-wider)] text-[var(--text-tertiary)]">Vista previa del lote</p>

              <div className="flex items-center justify-between">
                <GradoBadge grado={grado} />
                {humedadPct && <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${humOk ? "bg-[var(--data-success-100)] text-[var(--data-success-900)]" : "bg-[var(--data-warning-100)] text-[var(--data-warning-900)]"}`}><Droplets className="h-3.5 w-3.5" />{Number(humedadPct).toFixed(1)}%</span>}
              </div>

              {/* Cut test apilado */}
              <div className="mt-4">
                <div className="mb-1 flex items-center justify-between text-xs"><span className="text-[var(--text-tertiary)]">Prueba de corte</span><span className="font-mono tabular-nums text-[var(--text-secondary)]">índice ferm. {indice || 0}%</span></div>
                <div className="flex h-3 w-full overflow-hidden rounded-full bg-[var(--surface-sunken)]">
                  ...
                  ].map((s, i) => s.v > 0 && <div key={i} style={{ width: `${Math.min((s.v / Math.max(cutSum, 100)) * 100, 100)}%`, background: s.c }} />)}
                </div>
                {!hasCut && <p className="mt-1 text-xs text-[var(--text-tertiary)]">Cargá la prueba de corte para ver el grado.</p>}
              </div>

              {/* Liquidación */}
              <div className="mt-5 rounded-xl border-2 border-[var(--data-success-500)]/40 bg-[var(--data-success-50)] p-4">
                <p className="text-[length:var(--ts-2xs)] font-bold uppercase tracking-[var(--ls-wider)] text-[var(--data-success-700)]">Total a pagar al productor</p>
                <p className="mt-1 font-mono text-3xl font-extrabold tabular-nums text-[var(--data-success-700)]">S/ {liquidacion ? liquidacion.toFixed(2) : "0.00"}</p>
                <p className="mt-1 text-xs text-[var(--text-secondary)]">
                  {pesoKg ? `${Number(pesoKg).toFixed(2)} kg` : "— kg"} × S/ {((Number(precioPorKg) || 0) + (Number(premioPorKg) || 0)).toFixed(2)}/kg
                </p>
              </div>

              <p className="mt-3 text-xs text-[var(--text-tertiary)]">Grado I: mohoso ≤3% · pizarroso ≤3% · humedad ≤7.5%. Todo se recalcula al tipear.</p>
            </aside>
          </div>
        </div>
...
function Segmented({ value, onChange, options }: { value: string; onChange: (v: "humedo" | "seco") => void; options: { v: string; label: string }[] }) {
  return (
    <div className="flex h-11 rounded-lg border border-[var(--rule-base)] bg-[var(--surface-canvas)] p-1">
      {options.map((o) => (
        <button key={o.v} type="button" onClick={() => onChange(o.v as "humedo" | "seco")} className={`flex-1 rounded-md text-sm font-bold transition ${value === o.v ? "bg-[var(--surface-raised)] text-[var(--accent)] shadow-sm" : "text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"}`}>{o.label}</button>
      ))}
    </div>
  );
}
...
function GradoBadge({ grado }: { grado: "I" | "II" | "fuera_norma" | null }) {
  if (!grado) return <span className="inline-flex items-center rounded-full bg-[var(--surface-sunken)] px-3 py-1.5 text-sm font-bold text-[var(--text-tertiary)]">Sin clasificar</span>;
  const cls = grado === "I" ? "bg-[var(--data-success-100)] text-[var(--data-success-900)]" : grado === "II" ? "bg-[var(--data-warning-100)] text-[var(--data-warning-900)]" : "bg-[var(--data-error-100)] text-[var(--data-error-700)]";
  return <span className={`inline-flex items-center rounded-full px-3 py-1.5 text-base font-extrabold ${cls}`}>{GRADO_LABEL[grado]}</span>;
}
