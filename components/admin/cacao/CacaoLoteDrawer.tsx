...
interface LoteFull {
  ...
  premioPorKg: string | null;
  totalPagado: string | null;
  cutGranos: number | null;
  pctBienFermentado: string | null;
  pctVioleta: string | null;
  pctPizarroso: string | null;
  pctMohoso: string | null;
  granosPor100g: number | null;
  pctCascara: string | null;
  pctImpurezas: string | null;
...
interface Beneficio {
  ...
  fermTempMaxC: string | null;
  tipoFermentador: string | null;
  secDias: number | null;
  metodoSecado: string | null;
  humedadInicial: string | null;
  humedadFinal: string | null;
  pesoHumedoKg: string | null;
  pesoSecoKg: string | null;
  mermaPct: string | null;
  observaciones: string | null;
}
interface VentaLink {
  id: string;
...
export default function CacaoLoteDrawer({
  loteId,
  acopiador,
  onClose,
}: {
  ...
  const [ventas, setVentas] = useState<VentaLink[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetch(
          `/api/admin/cacao?view=lote-detail&id=${encodeURIComponent(loteId)}`,
          { credentials: "include" },
        );
        if (!r.ok)
        ...
        if (cancel) return;
        setLote(d.lote ?? null);
        setBeneficio(d.beneficio ?? null);
        setVentas(d.ventas ?? []);
        setProductorTel(d.productorTelefono ?? null);
      } catch (e) {
    ...
    ? [
        {
          ...
          sub: "marrón · fino aroma",
          v: lote.pctBienFermentado,
      ...
      footer={
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3.5">
            ...
            <button
              ...
              className="inline-flex h-10 items-center gap-2 rounded-xl bg-[var(--accent)] px-4 text-sm font-semibold text-white hover:opacity-90"
            >
          ...
          {lote && !loading && (
            <>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-base font-bold text-[var(--text-primary)]">
                    {lote.productorNombre ?? "Productor no asignado"}
                  </p>
                  <p className="text-[var(--text-tertiary)]">
                    {fdate(lote.fecha)} · {lote.variedad ?? "—"} ·{" "}
                    {lote.tipoGrano === "humedo" ? "húmedo" : "seco"}
                  </p>
                </div>
              ...
              <Section icon={Coins} title="Liquidación al productor">
                ...
                <Row
                  label="Precio por kg"
                  value={lote.precioPorKg ? `S/ ${n2(lote.precioPorKg)}` : "—"}
                />
                <Row
                  label="Premio por kg"
                  value={lote.premioPorKg ? `S/ ${n2(lote.premioPorKg)}` : "—"}
              ...
              <Section icon={Beaker} title="Calidad — prueba de corte (NTP-ISO 1114)">
                {hasCut ? (
                  <div className="space-y-2.5">
                    {cut.map((c) => (
                      <CutBar key={c.label} {...c} />
                    ))}
                    <div className="grid grid-cols-3 gap-2 pt-1">
                      <MiniStat
                        label="Índice ferm."
                        value={
                          lote.indiceFermentacion
                            ? `${Number(lote.indiceFermentacion).toFixed(0)}%`
                            : "—"
                        }
                      />
                      <MiniStat
                        label="Humedad"
                        value={pct(lote.humedadPct)}
                        warn={lote.humedadPct != null && Number(lote.humedadPct) > 7}
                      />
                      <MiniStat
                        label="Granos eval."
                        value={lote.cutGranos != null ? String(lote.cutGranos) : "—"}
                      />
                    </div>
                  </div>
                ) : (
                  <p className="text-[var(--text-tertiary)]">
                    Sin prueba de corte registrada. Humedad: {pct(lote.humedadPct)}.
                  </p>
                )}
                {(lote.granosPor100g != null || lote.pctCascara || lote.pctImpurezas) && (
                  <div className="mt-2 grid grid-cols-3 gap-2 border-t border-[var(--rule-soft)] pt-2">
                    <MiniStat
                      label="Granos/100g"
                      value={lote.granosPor100g != null ? String(lote.granosPor100g) : "—"}
                    />
              ...
              {beneficio ? (
                <Section icon={Droplets} title="Beneficio (fermentación + secado)">
                  <div className="mb-2">
                    <span className="inline-flex rounded-full bg-[var(--surface-sunken)] px-2.5 py-1 text-xs font-bold capitalize text-[var(--text-secondary)]">
                      {beneficio.estado}
                    </span>
                  </div>
                  <Row
                    label="Fermentación"
                    value={`${beneficio.fermDias ?? "—"} días · ${beneficio.fermVolteos ?? "—"} volteos${beneficio.tipoFermentador ? ` · ${beneficio.tipoFermentador}` : ""}`}
                  />
                  {beneficio.fermTempMaxC && (
                    <Row
                      label="Temp. máx."
                      value={`${Number(beneficio.fermTempMaxC).toFixed(0)} °C`}
                    />
                  )}
                  <Row
                    label="Secado"
                    value={`${beneficio.secDias ?? "—"} días${beneficio.metodoSecado ? ` · ${beneficio.metodoSecado}` : ""}`}
                  />
                  <Row
                    label="Humedad inicial → final"
                    value={`${pct(beneficio.humedadInicial)} → ${pct(beneficio.humedadFinal)}`}
                  />
                  <Row
                    label="Peso húmedo → seco"
                    value={`${n2(beneficio.pesoHumedoKg)} → ${n2(beneficio.pesoSecoKg)} kg`}
                    strong
                  />
                  {beneficio.mermaPct && (
                    <div className="mt-1 flex items-center justify-between rounded-xl bg-[var(--surface-sunken)] px-3 py-2">
                      ...
                      <span className="font-mono font-extrabold tabular-nums text-[var(--text-primary)]">
                        {pct(beneficio.mermaPct)}
                      </span>
                    </div>
                  )}
                </Section>
              ...
              {/* Trazabilidad forward: ventas que salieron de este lote (lote→venta) */}
              <Section icon={Coins} title="Ventas de este lote">
                {ventas.length === 0 ? (
                  <p className="text-[var(--text-tertiary)]">
                  ...
                  <div className="space-y-1.5">
                    {ventas.map((v) => (
                      <div
                        key={v.id}
                        className="flex items-center justify-between gap-3 rounded-lg bg-[var(--surface-sunken)] px-3 py-2"
                      >
              ...
              {(lote.destino || lote.observaciones) && (
                <Section icon={Scale} title="Trazabilidad y notas">
                  ...
                  {lote.observaciones && (
                    <p className="text-[var(--text-secondary)]">{lote.observaciones}</p>
                  )}
                </Section>
              )}
                ...
                <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[var(--text-secondary)]">
                      Imprimilo en el saco. El comprador escanea y ve origen y calidad — sin
                      precios.
                    </p>
...
function Row({
  ...
  return (
    <div className="flex items-baseline justify-between gap-3 py-0.5">
      ...
      <span
        className={`text-right font-mono tabular-nums ${strong ? "font-bold text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}
      >
        {value}
      </span>
    </div>
...
function MiniStat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className="rounded-xl bg-[var(--surface-sunken)] px-3 py-2 text-center">
      ...
      <div
        className={`font-mono text-base font-extrabold tabular-nums ${warn ? "text-[var(--data-warning-700)]" : "text-[var(--text-primary)]"}`}
      >
        {value}
      </div>
    </div>
    ...
    <div>
      <div className="mb-1 flex items-baseline justify-between">
        <span className="text-[var(--text-secondary)]">
          {label} <span className="text-xs text-[var(--text-tertiary)]">{sub}</span>
        </span>
        <span className="font-mono text-sm font-bold tabular-nums text-[var(--text-primary)]">
          {v == null || v === "" ? "—" : `${val.toFixed(0)}%`}
        </span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-[var(--surface-sunken)]">
        <div className={`h-full rounded-full ${bar}`} style={{ width: `${Math.min(val, 100)}%` }} />
      </div>
    </div>
  );
}
...
function GradoBadge({ grado }: { grado: string | null }) {
  if (!grado)
    return (
      <span className="rounded-full bg-[var(--surface-sunken)] px-2.5 py-1 text-xs font-bold text-[var(--text-tertiary)]">
        sin clasificar
      </span>
    );
