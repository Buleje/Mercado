...
export default function CacaoCampoAnalisis({ onOpenParcela }: { onOpenParcela: (id: string) => void }) {
  ...
  if (secciones.length === 0) return <div className="rounded-2xl border-2 border-dashed border-[var(--rule-base)] bg-[var(--surface-raised)] p-12 text-center text-[var(--text-tertiary)]"><span className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-2xl bg-[var(--accent-soft)] text-[var(--accent)]"><TrendingUp className="h-7 w-7" /></span><p className="text-base font-bold text-[var(--text-primary)]">Sin datos para analizar</p><p className="mx-auto mt-1 max-w-sm text-sm">Registrá cosechas y costos de labores en tus secciones y acá verás rendimiento, ingresos y margen por sección.</p></div>;
