...
export default function CacaoAlertsBell({ onNavigate }: { onNavigate: (view: CacaoView) => void }) {
  ...
  const precioAlerta = useMemo<Alert | null>(() => {
    ...
    return {
      ...
      detail: `La referencia de mercado (${formatCurrency(precioMercado)}/kg) superó tu objetivo de ${formatCurrency(config.precioAlertaPenKg)}/kg. Buen momento para vender.`,
      view: "mercado",
    };
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`relative inline-flex h-12 items-center gap-2 rounded-2xl border-2 px-3.5 text-sm font-semibold transition ${count ? "border-[var(--data-warning-500)] bg-[var(--data-warning-50)] text-[var(--data-warning-700)]" : "border-[var(--rule-base)] bg-[var(--surface-raised)] text-[var(--text-primary)] hover:bg-[var(--surface-canvas)]"}`}
      >
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-dropdown" onClick={() => setOpen(false)} aria-hidden />
          <div className="absolute right-0 z-modal mt-2 w-[min(92vw,380px)] overflow-hidden rounded-2xl border border-[var(--rule-base)] bg-[var(--surface-raised)] shadow-xl">
