...
interface Beneficio {
  ...
  fermInicio: string | null;
  secInicio: string | null;
  createdAt: string;
}
...
function alertaOf(b: Beneficio) {
  const dias =
    b.estado === "fermentando"
      ? daysSince(b.fermInicio ?? b.createdAt)
      : b.estado === "secando"
        ? daysSince(b.secInicio ?? b.createdAt)
        : null;
...
export default function CacaoBeneficio() {
  ...
  const [bitacora, setBitacora] = useState<{ id: string; loteCode: string | null } | null>(null);
  ...
  async function annul() {
    ...
    try {
      const r = await fetch("/api/admin/cacao", {
        ...
        credentials: "include",
        body: JSON.stringify({ action: "annul_beneficio", id: annulId }),
      });
  ...
  function openAdvance(b: Beneficio) {
    setAdvance(b);
    setAdvPesoSeco(b.pesoSecoKg ?? "");
    setAdvHumedad(b.humedadFinal ?? "");
    setAdvMetodo(b.metodoSecado ?? "");
  }
  async function doAdvance() {
    ...
    try {
      ...
      if (advance.estado === "secando") {
        body.pesoSecoKg = advPesoSeco === "" ? null : Number(advPesoSeco);
        if (advHumedad !== "") body.humedadFinal = Number(advHumedad);
      }
      const r = await fetch("/api/admin/cacao", {
        method: "PATCH",
        headers: csrfHeaders({ "Content-Type": "application/json" }),
  ...
  const view = useMemo(() => {
    ...
    return items.filter(
      (b) =>
        (filter === "todos" ||
          (filter === "atencion" ? alertaOf(b).nivel !== "ok" : b.estado === filter)) &&
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `cacao-beneficios-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          label="Terminados"
          value={String(kpis.terminados)}
          subValue="beneficios completos"
          icon={Sun}
          emphasis="success"
        />
        <StatCard
          label="Kg seco producido"
          value={`${n2(kpis.kgSeco)} kg`}
          subValue="de beneficios terminados"
          icon={Scale}
          emphasis="success"
        />
        <StatCard
      ...
      {kpis.atencion > 0 && filter !== "atencion" && (
        ...
        >
          ...
          <span className="flex-1">
            <strong>
              {kpis.atencion} {kpis.atencion === 1 ? "lote requiere" : "lotes requieren"} atención
            </strong>{" "}
            — llevan demasiados días en fermentación o secado.
          </span>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--surface-sunken)] text-left">
            <tr>
              ...
              <Th>Fermentación</Th>
              <Th>Secado</Th>
              <Th className="text-right">Hum. final</Th>
              <Th className="text-right">Húmedo→Seco</Th>
                ...
                >
                  ...
                  <Td>
                    <span className="inline-flex items-center gap-1.5">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${est.cls}`}
                      >
                        {est.label}
                        {al.dias != null && b.estado !== "terminado" ? ` · ${al.dias}d` : ""}
                      </span>
                      {al.nivel !== "ok" && (
                        <span title={al.motivo} className="inline-flex">
                          <AlertTriangle
                            className={`h-4 w-4 ${ALERTA_CLS[al.nivel]}`}
                            aria-label={al.motivo}
                          />
                        </span>
                      )}
                    </span>
                  ...
                  <Td className="text-[var(--text-secondary)]">
                    {b.fermDias != null ? `${b.fermDias}d` : "—"}
                    {b.fermVolteos != null ? ` · ${b.fermVolteos} volteos` : ""}
                    {b.tipoFermentador ? ` · ${b.tipoFermentador}` : ""}
                  </Td>
                  <Td className="text-[var(--text-secondary)]">
                    {b.secDias != null ? `${b.secDias}d` : "—"}
                    {b.metodoSecado ? ` · ${b.metodoSecado}` : ""}
                  </Td>
                  <Td className="text-right font-mono tabular-nums">
                    <span
                      className={
                        b.humedadFinal && Number(b.humedadFinal) > 7
                          ? "text-[var(--data-warning-700)]"
                          : "text-[var(--text-secondary)]"
                      }
                    >
                      {b.humedadFinal ? `${Number(b.humedadFinal).toFixed(1)}%` : "—"}
                    </span>
                  </Td>
                  <Td className="text-right font-mono tabular-nums text-[var(--text-secondary)]">
                    {b.pesoHumedoKg ? n2(b.pesoHumedoKg) : "—"} →{" "}
                    {b.pesoSecoKg ? n2(b.pesoSecoKg) : "—"}
                  </Td>
                  <Td className="text-right font-mono font-bold tabular-nums text-[var(--text-primary)]">
                    {b.mermaPct ? `${Number(b.mermaPct).toFixed(1)}%` : "—"}
                  </Td>
                  <Td className="text-right font-mono tabular-nums text-[var(--text-secondary)]">
                    {rend != null ? `${rend}%` : "—"}
                  </Td>
                  <Td className="text-right">
                    <div className="inline-flex items-center justify-end gap-1.5">
                      {b.estado === "fermentando" && (
                        <button
                          type="button"
                          title="Bitácora diaria de fermentación"
                          onClick={(e) => {
                            e.stopPropagation();
        ...
        {loading && items.length === 0 ? (
          ...
          (search || filter !== "todos" ? (
            <div className="p-12 text-center text-[var(--text-tertiary)]">
              ...
              <p className="text-base font-medium">Sin beneficios para ese filtro.</p>
            </div>
          ) : (
            <div className="p-12 text-center text-[var(--text-tertiary)]">
      ...
      {bitacora && (
        <CacaoBitacoraModal
          beneficioId={bitacora.id}
          loteCode={bitacora.loteCode}
              ...
              <div className="bg-[var(--surface-raised)] p-5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-sm text-[var(--text-tertiary)]">
                      {aSecado
                        ...
                        : "Cierra el secado. Ingresa el peso seco final para calcular la merma y el rendimiento del lote."}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <Field
                        label={
                          <>
                            Peso seco final (kg){" "}
                            <span className="text-[var(--data-error-600)]">*</span>
                          </>
                        }
                        labelClassName="mb-1.5 block text-xs font-bold text-[var(--text-secondary)]"
                      >
                        <input
                          autoFocus
                          type="number"
                      ...
                      <Field
                        label={
                          <>
                            Humedad final (%){" "}
                            <span className="font-normal text-[var(--text-tertiary)]">opc.</span>
                          </>
                        }
                        labelClassName="mb-1.5 block text-xs font-bold text-[var(--text-secondary)]"
                      >
                    ...
                    {pesoHum != null && (
                      <p className="text-xs text-[var(--text-tertiary)]">
                        Húmedo registrado:{" "}
                        <strong className="text-[var(--text-secondary)]">{n2(pesoHum)} kg</strong>
                      </p>
                    )}
                    {(mermaPrev != null || rendPrev != null) && (
                      <div className="flex gap-2">
                        {mermaPrev != null && (
                          <div className="flex-1 rounded-xl bg-[var(--surface-sunken)] px-3 py-2">
                            <div className="text-xs text-[var(--text-tertiary)]">Merma</div>
                            <div className="font-mono text-base font-bold tabular-nums text-[var(--text-primary)]">
                              {mermaPrev.toFixed(1)}%
                            </div>
                          </div>
                        )}
                        {rendPrev != null && (
                          <div className="flex-1 rounded-xl bg-[var(--surface-sunken)] px-3 py-2">
                            <div className="text-xs text-[var(--text-tertiary)]">Rendimiento</div>
                            <div className="font-mono text-base font-bold tabular-nums text-[var(--text-primary)]">
                              {rendPrev}%
                            </div>
                          </div>
                    ...
                    {advHumedad !== "" && Number(advHumedad) > 7.5 && (
                      <p className="flex items-center gap-1.5 text-xs font-medium text-[var(--data-warning-700)]">
                        <AlertTriangle className="h-3.5 w-3.5" />
                        Humedad sobre 7.5% — fuera de la meta NTP (≤7%).
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="mt-5 flex justify-end gap-2">
                  ...
                  >
                    ...
                    {advBusy ? "Guardando…" : aSecado ? "Pasar a secado" : "Terminar beneficio"}
                  </button>
                </div>
              </div>
            </AdminModal>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-sm text-[var(--text-tertiary)]">
                  ...
                  el historial. El lote vuelve a estar disponible para beneficiar.
                </p>
              </div>
            </div>
            <div className="mt-5 flex justify-end gap-2">
