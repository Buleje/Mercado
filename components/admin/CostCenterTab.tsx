    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      {view === "centros" && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
          {centerStats.map(c => (
            <div key={c.id} className={cn("bg-white dark:bg-card border rounded-xl p-3 sm:p-5", c.pct > 100 ? "border-red-200 dark:border-red-800" : c.pct > 85 ? "border-amber-200 dark:border-amber-800" : "border-[var(--rule-base)] dark:border-card-border")}>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-card border border-[var(--rule-base)] dark:border-card-border rounded-xl p-3 sm:p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <select value={formAlloc.centerId} onChange={e => setFormAlloc(p => ({ ...p, centerId: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">{centers.filter(c => c.active).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Categoría</label>
                  <select value={formAlloc.category} onChange={e => setFormAlloc(p => ({ ...p, category: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">{EXPENSE_CATEGORIES.map(c => <option key={c}>{c}</option>)}</select>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Fecha</label>
                  <input type="date" value={formAlloc.date} onChange={e => setFormAlloc(p => ({ ...p, date: e.target.value }))} className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Monto (S/)</label>
                  <input type="number" value={formAlloc.amount || ""} onChange={e => setFormAlloc(p => ({ ...p, amount: parseFloat(e.target.value) || 0 }))} min="0" step="10" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
                </div>
                <div className="col-span-2">
                  <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Concepto *</label>
                  <input type="text" value={formAlloc.concept} onChange={e => setFormAlloc(p => ({ ...p, concept: e.target.value }))} placeholder="Descripción del gasto..." className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-muted block mb-1">Referencia</label>
                  <input type="text" value={formAlloc.reference} onChange={e => setFormAlloc(p => ({ ...p, reference: e.target.value }))} placeholder="OC-2026-XXX" className="w-full text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
                </div>
              </div>
              <div className="flex flex-wrap gap-2 justify-end">
                <button onClick={() => setShowFormAlloc(false)} className="px-2 sm:px-4 py-1.5 sm:py-2 text-sm rounded-lg border border-[var(--rule-base)] dark:border-card-border text-gray-600 dark:text-muted">Cancelar</button>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Concepto, referencia..." className="w-full pl-9 pr-3 py-2 text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
            </div>
            <select value={filterCenter} onChange={e => setFilterCenter(e.target.value)} className="text-sm border border-[var(--rule-base)] dark:border-card-border rounded-lg px-3 py-2 bg-white dark:bg-surface text-gray-700 dark:text-foreground">
