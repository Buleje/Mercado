    ...
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => exportToCSV(filteredAllocations.map(a => ({ centro: centerName(a.centerId), fecha: a.date, concepto: a.concept, categoria: a.category, monto: a.amount, referencia: a.reference, notas: a.notes })), "centros-costo")} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[var(--rule-base)] dark:border-[var(--rule-base)] bg-white dark:bg-surface text-sm font-semibold text-[var(--text-primary)] dark:text-[var(--text-primary)] hover:bg-[var(--surface-alt)] dark:hover:bg-accent transition-colors">
      ...
      <div className="flex flex-wrap gap-1 bg-[var(--surface-sunken)] dark:bg-surface rounded-xl p-1 w-fit">
        {([["centros", "Centros"], ["asignaciones", "Asignaciones"]] as const).map(([v, label]) => (
          <button key={v} onClick={() => setView(v)} className={cn("px-2 sm:px-4 py-1.5 sm:py-2 text-sm font-semibold rounded-lg transition-colors", view === v ? "bg-[var(--surface-raised)] text-[var(--text-primary)] dark:text-[var(--text-primary)] " : "text-[var(--text-secondary)] dark:text-muted hover:text-[var(--text-primary)]")}>
            {label}
          </button>
        ))}
      </div>
      ...
      {view === "centros" && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
          {centerStats.map(c => (
            <div key={c.id} className={cn("bg-[var(--surface-raised)] border rounded-xl p-3 sm:p-5", c.pct > 100 ? "border-[var(--data-error-500)] dark:border-[var(--data-error-500)]" : c.pct > 85 ? "border-[var(--data-warning-500)] dark:border-[var(--data-warning-500)]" : "border-[var(--rule-base)] dark:border-[var(--rule-base)]")}>
        ...
        <div className="space-y-6">
          ...
          {showFormAlloc && (
            <div className="bg-[var(--surface-raised)] border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-xl p-3 sm:p-5 space-y-4">
              <div className="flex items-center justify-between">
                <CardTitle className="font-bold text-[var(--text-primary)] dark:text-[var(--text-primary)] text-sm flex flex-wrap items-center gap-2"><Plus className="h-4 w-4 text-primary" /> Asignar gasto a centro</CardTitle>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Concepto, referencia..." className="w-full pl-9 pr-3 py-2 text-sm border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-lg bg-white dark:bg-surface text-[var(--text-primary)] dark:text-[var(--text-primary)]" />
            </div>
            <select value={filterCenter} onChange={e => setFilterCenter(e.target.value)} className="text-sm border border-[var(--rule-base)] dark:border-[var(--rule-base)] rounded-lg px-3 py-2 bg-white dark:bg-surface text-[var(--text-primary)] dark:text-[var(--text-primary)]">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-card-border">
                  ...
                  {filteredAllocations.map(a => (
                    <tr key={a.id} className="hover:bg-[var(--surface-alt)]/50 dark:hover:bg-surface/30">
                      ...
                      <td className="px-2 sm:px-4 py-2 sm:py-3 font-semibold text-[var(--text-primary)] dark:text-[var(--text-primary)] text-xs">{centerName(a.centerId)}</td>
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-[var(--text-primary)] dark:text-[var(--text-primary)]">{a.concept}</td>
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-xs text-[var(--text-secondary)] dark:text-muted">{a.category}</td>
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-right font-bold text-[var(--text-primary)] dark:text-[var(--text-primary)]">{fmt(a.amount)}</td>
                ...
                {filteredAllocations.length > 0 && (
                  <tfoot className="bg-[var(--surface-alt)] dark:bg-surface/50 border-t border-[var(--rule-base)] dark:border-[var(--rule-base)]">
                    <tr>
                      ...
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-right font-extrabold text-[var(--text-primary)] dark:text-[var(--text-primary)]">{fmt(filteredAllocations.reduce((s, a) => s + a.amount, 0))}</td>
                      <td />
                    </tr>
                  </tfoot>
                )}
