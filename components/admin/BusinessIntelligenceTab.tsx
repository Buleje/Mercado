  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {KPIS.map(kpi => (
          <div key={kpi.label} className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-xl p-4">
            ...
            <div className="flex items-center gap-1 mt-1">
              {kpi.trend === "up" ? <ArrowUpRight className="h-3 w-3 text-emerald-500" /> : <ArrowDownRight className="h-3 w-3 text-red-500" />}
              <span className={cn("text-xs font-bold", kpi.trend === "up" ? "text-emerald-500" : "text-red-500")}>{kpi.pct > 0 ? "+" : ""}{kpi.pct}%</span>
              <span className="text-xs text-gray-400 ml-1">vs. antes</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-xl p-3 sm:p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
          ...
          <div><p className="text-xs text-gray-400 mb-1">Ticket promedio</p><p className="text-xl font-extrabold text-gray-800 dark:text-foreground">{fmt(forecast.avgTicket)}</p></div>
        </div>
        {/* Confidence bar */}
        <div className="mt-4 relative h-6 bg-gray-100 dark:bg-surface rounded-full overflow-hidden">
          ...
          <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-gray-700 dark:text-foreground">{fmt(forecast.predictedRevenue)}</span>
        </div>
      </div>
              ...
              {sortedTrends.map(t => (
                <tr key={t.category} className="border-t border-gray-100 dark:border-card-border">
                  ...
                  <td className="py-2.5 pr-4">
                    <span className={cn("font-bold flex items-center gap-1", t.growthPct >= 0 ? "text-emerald-500" : "text-red-500")}>
                      {t.growthPct >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                      {t.growthPct > 0 ? "+" : ""}{t.growthPct}%
                    </span>
                  </td>
                  <td className="py-2.5 text-gray-600 dark:text-muted">{t.topProduct}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {ANOMALIES.map(a => {
            ...
            return (
              <div key={a.id} className={cn("rounded-xl p-3 border", sevMeta[a.severity])}>
                ...
                <div className="text-xs text-gray-500 mt-1">Valor: <b>{a.value.toLocaleString()}</b> (esperado: {a.expected.toLocaleString()})</div>
              </div>
            );
          })}
        </div>
