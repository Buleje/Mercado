...
function ModuleTooltip() {
  ...
  return (
    <div className="relative inline-block">
      <button type="button" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} onFocus={() => setOpen(true)} onBlur={() => setOpen(false)} className="text-gray-400 hover:text-primary transition-colors focus:outline-none" aria-label="Ayuda sobre Lotes y Vencimientos">
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: "Total lotes", count: stats.total, color: "text-gray-700 dark:text-foreground", bg: "bg-gray-50 dark:bg-surface/50" },
          { label: "Vigentes", count: stats.vigente, color: "text-emerald-700 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/30" },
          { label: "Por vencer", count: stats.por_vencer, color: "text-amber-700 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-950/30" },
      ...
      {/* Form */}
      {showForm && (
        <div id="batch-form-top" className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-xl p-3 sm:p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            <button onClick={() => { setEditingId(null); setShowForm(false); setForm({ ...EMPTY_FORM }); }} className="px-2 sm:px-4 py-1.5 sm:py-2 text-sm rounded-lg border border-gray-200 dark:border-card-border text-gray-600 dark:text-muted">Cancelar</button>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar producto, lote, proveedor..." className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 dark:border-card-border rounded-lg bg-white dark:bg-surface text-gray-700 dark:text-foreground" />
        </div>
        <div className="flex gap-2 flex-wrap">
          {(["todos", "vigente", "por-vencer", "critico", "vencido"] as const).map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} className={cn("px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors capitalize", filterStatus === s ? "bg-primary text-white" : "bg-white dark:bg-card border border-gray-200 dark:border-card-border text-gray-600 dark:text-muted hover:bg-gray-50 dark:hover:bg-accent")}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-card-border">
              {processed.length === 0 && (
                <tr><td colSpan={11} className="px-4 py-8 text-center text-gray-400 dark:text-muted text-sm">No se encontraron lotes con los filtros seleccionados.</td></tr>
              )}
              {processed.map(b => {
                  ...
                  <tr key={b.id} className={cn("hover:bg-gray-50/50 dark:hover:bg-surface/30 transition-colors", b.status === "vencido" ? "bg-red-50/30 dark:bg-red-950/10" : b.status === "critico" ? "bg-orange-50/30 dark:bg-orange-950/10" : "")}>
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => handleDelete(b.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors" title="Eliminar lote">
