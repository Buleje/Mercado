...
export default function PayrollTab() {
  ...
  const STATUS_META: Record<PayrollStatus, { label: string; color: string; bg: string }> = {
    pendiente:   { label: "Pendiente",   color: "text-amber-700 dark:text-amber-400",   bg: "bg-amber-100 dark:bg-amber-900/30"   },
    pagado:      { label: "Pagado",      color: "text-emerald-700 dark:text-emerald-400",bg: "bg-emerald-100 dark:bg-emerald-900/30"},
    observacion: { label: "Observación", color: "text-red-700 dark:text-red-400",        bg: "bg-red-100 dark:bg-red-900/30"        },
  };
                  ...
                  <tr key={p.id} className="hover:bg-gray-50/50 dark:hover:bg-surface/30">
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3">
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        ...
                        {p.status === "pendiente" && (
                          <button onClick={() => markPaid(p.id)} className="px-2.5 py-1 text-xs rounded-lg bg-emerald-500 text-white font-semibold hover:bg-emerald-600 transition-colors">
                            Pagar
                          </button>
                        )}
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setDetail(null)}>
          <div className="bg-white dark:bg-card border border-gray-200 dark:border-card-border rounded-2xl shadow-2xl p-3 sm:p-6 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-gray-50 dark:bg-surface rounded-xl p-4 text-sm space-y-2">
              <p className="font-bold text-gray-900 dark:text-foreground">{detail.employeeName}</p>
              <p className="text-xs text-gray-500 dark:text-muted">{detail.role} · {detail.department} · {MONTH_LABELS[detail.month] ?? detail.month}</p>
            </div>
...
function Row({ label, value, bold, accent, vcolor }: { label: string; value: string; bold?: boolean; accent?: boolean; vcolor?: string }) {
  return (
    <div className="flex justify-between">
      <span className={cn("text-gray-500 dark:text-muted", bold && "font-bold text-gray-700 dark:text-foreground")}>{label}</span>
      <span className={cn("font-semibold", bold && "font-extrabold", accent ? "text-emerald-600 text-base" : vcolor ?? "text-gray-700 dark:text-foreground")}>{value}</span>
    </div>
  );
}
