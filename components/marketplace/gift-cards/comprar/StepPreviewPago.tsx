      ...
      <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="mt-4 flex items-baseline justify-between border-t border-gray-100 pt-4 dark:border-gray-800">
            <span className="text-sm font-semibold text-[var(--text-secondary)] dark:text-gray-400">
              Total
            </span>
          ...
          <button
            ...
            onClick={onBack}
            disabled={submitting}
            className="mt-2 inline-flex w-full items-center justify-center gap-1.5 py-2 text-xs font-semibold text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)] disabled:opacity-40 dark:text-gray-400 dark:hover:text-white"
          >
        ...
        <ul className="space-y-1.5 text-xs text-[var(--text-secondary)] dark:text-gray-400">
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <dt className="text-[var(--text-secondary)] dark:text-gray-400">{label}</dt>
      <dd className="truncate text-right font-medium text-[var(--text-primary)] dark:text-white">
        {value}
      </dd>
    </div>
  );
