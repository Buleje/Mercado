...
function BannerInner({ banner, type }: { banner: PromoBanner; type: BannerType }) {
  ...
  if (type === "promo" && banner.promo) {
    ...
    return (
      <div
        className="relative overflow-hidden rounded-2xl aspect-[16/9] sm:aspect-auto sm:h-[240px] lg:h-[290px] xl:h-[320px] flex items-stretch px-4 sm:px-6 gap-3 sm:gap-4 border border-[var(--rule-soft)]"
  ...
  if (type === "image") {
    return (
      <div
        className="relative overflow-hidden rounded-2xl aspect-[16/9] sm:aspect-auto sm:h-[240px] lg:h-[290px] xl:h-[320px] flex items-center justify-center border border-[var(--rule-soft)]"
        style={{ background: bgGradient }}
      >
        {hasImage && banner.imageUrl && (
          <Image
