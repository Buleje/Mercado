...
import {
  ...
  BLOQUES_POR_HOJA, FUENTES, HEAD_COLS, PAGINA, TEXTO_LEGAL, ETIQUETAS_FIRMA,
  type Anexo04, type Anexo04Opts, type DatosAnexo04, type GeoHoja, type HojaAnexo04,
} from "./anexo04-serfor";
...
function dibujarCabecera(doc: jsPDF, g: GeoHoja, datos: DatosAnexo04, hojaActual: HojaAnexo04) {
  const m = PAGINA.margen;
  doc.setDrawColor(0); doc.setLineWidth(LINEA);

  doc.setFillColor(...VERDE);
  doc.rect(m, g.yBanner, g.contentW, g.hBanner, "FD");
  celdaTexto(doc, "ANEXO N° 04", m, g.yBanner - 2, g.contentW, g.hBanner / 2, FUENTES.banner, "center", true);
  celdaTexto(doc, '"LISTA DE PRODUCTOS TRANSFORMADOS"', m, g.yBanner + g.hBanner / 2 - 2, g.contentW, g.hBanner / 2, FUENTES.bannerTitulo, "center", true);

  doc.setFillColor(...VERDE);
  doc.rect(m, g.yInstr, g.contentW, g.hInstr, "FD");
  celdaTexto(doc, "Para el llenado del presente Anexo se utilizarán las instrucciones adjuntas.", m, g.yInstr, g.contentW, g.hInstr, FUENTES.instrucciones, "left");
  ...
  celdaTexto(doc, "LISTA DE PRODUCTOS TRANSFORMADOS", m + 150, g.yInfo, g.contentW - 340, 24, FUENTES.titulo, "center", true);

  const xr = m + g.contentW - 200;
  const campo = (i: number, label: string, valor: string) => {
    const y = g.yInfo + 20 + i * 15;
    doc.setFont("helvetica", "bold"); doc.setFontSize(FUENTES.campos); doc.setTextColor(0);
    doc.text(label, xr, y);
    const lw = doc.getStringUnitWidth(label) * FUENTES.campos;
    doc.setFont("helvetica", "normal");
    doc.text(valor || "…………………………", xr + lw + 4, y);
  };
  campo(0, "(1) N°:", datos.numero);
  campo(1, "(2) GTF N°:", datos.gtf);
  /* El volumen de ESTA hoja, no el del anexo (el total del documento va al pie
     cuando hay más de una): cada hoja se muestra sola en un control. */
  campo(2, "(3) VOLUMEN TOTAL:", `${fmtAnexo(hojaActual.totalM3)} m³`);
}
...
function dibujarBloque(doc: jsPDF, g: GeoHoja, i: number, hoja: HojaAnexo04) {
  ...
  HEAD_COLS.forEach((h, j) => {
    ...
    celdaTexto(doc, h, x, g.yTblHead, w, g.hTblHead, FUENTES.tablaHead, "center", true);
  });
  ...
  for (let f = 0; f < hoja.filasPorBloque; f++) {
    ...
    celdas.forEach((c, j) => {
      ...
      celdaTexto(doc, c, x, y, w, g.hFila, FUENTES.celda, j === 5 ? "right" : "center", false);
    });
  }
  ...
  celdaTexto(doc, "(11) SUB TOTAL", x0, yS, wLabel, g.hSub, FUENTES.subtotalLabel, "right", true);
  const xV = g.xCol(i, 5);
  doc.setLineWidth(0.6);
  doc.rect(xV, yS, g.cols[5], g.hSub);
  doc.setLineWidth(LINEA);
  celdaTexto(doc, fmtAnexo(b?.subtotal ?? 0), xV, yS, g.cols[5], g.hSub, FUENTES.subtotalValor, "right", true);
}
...
function dibujarPie(doc: jsPDF, g: GeoHoja, datos: DatosAnexo04, anexo: Anexo04, hoja: number, hojas: number) {
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(FUENTES.observaciones); doc.setTextColor(0);
  doc.text("(12) OBSERVACIONES:", m + 4, g.yObs + 12);
  if (datos.observaciones) {
    doc.setFont("helvetica", "normal"); doc.setFontSize(FUENTES.observaciones);
  ...
  g.yFirmas.forEach((y, i) => {
    ...
    if (valores[i]) {
      doc.setFont("helvetica", "normal"); doc.setFontSize(FUENTES.firma);
      doc.text(valores[i], xF2, y - 3, { align: "right" });
    }
    doc.setFont("helvetica", "bold"); doc.setFontSize(FUENTES.firmaLabel);
    doc.text(ETIQUETAS_FIRMA[i], xF2, y + 8, { align: "right" });
  });

  doc.setFont("helvetica", "normal"); doc.setFontSize(FUENTES.legal); doc.setTextColor(60);
  ...
  const pie = hojas > 1 ? `Anexo completo: ${fmtAnexo(anexo.totalM3)} m³ · Hoja ${hoja} de ${hojas}` : `Hoja ${hoja} de ${hojas}`;
  doc.text(pie, m + g.contentW, g.yLegal + 24, { align: "right" });
  doc.setTextColor(0);
}
...
function dibujarHoja(doc: jsPDF, hoja: HojaAnexo04, datos: DatosAnexo04, anexo: Anexo04, nro: number, total: number) {
  ...
  dibujarCabecera(doc, g, datos, hoja);
...
function agregarAnexo(doc: jsPDF, rows: PiezaCubicada[], datos: DatosAnexo04, opts: Anexo04Opts, esPrimero: boolean) {
...
export async function abrirAnexo04PDF(rows: PiezaCubicada[], datos: DatosAnexo04, opts: Anexo04Opts = {}): Promise<void> {
  const doc = await construirDoc(rows, datos, opts);
