...
export interface EntradaExcepciones {
  materiaPrima: { pendienteM3: number };
...
export function excepcionesDeSaldo(input: EntradaExcepciones): Excepcion[] {
  ...
  const negativas = input.porEspecie.filter((e) => e.saldoM3 < -EPS).sort((a, b) => a.saldoM3 - b.saldoM3);
  if (negativas.length > 0) {
    const total = negativas.reduce((a, e) => a + Math.abs(e.saldoM3), 0);
    fuera.push({
      clave: "mp-negativa",
      tono: "error",
      titulo: `${negativas.length} ${plural(negativas.length, "especie", "especies")} en negativo · ${m3(total)} sin respaldo`,
      detalle:
        "Se transformó más volumen del que ingresó validado. O falta validar un ingreso, o una corrida cargó de más. Hasta corregirlo, el libro no cuadra ante SERFOR.",
      items: negativas.map((e) => `${e.especie} (${m3(e.saldoM3)})`),
