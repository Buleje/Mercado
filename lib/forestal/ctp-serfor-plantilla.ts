...
export async function descargarPlantillaSerfor(nRegistro = N_REGISTRO_DEMO): Promise<void> {
  ...
  const lineas: [string, boolean][] = [
    ...
    ["Si ya bajaste el Excel del SNIFFS, sube ese directamente: no hace falta pasarlo acá.", false],
    ["", false],
    ["Cómo llenarla", true],
    ["1. Una hoja por sección. Llena solo las que necesites — las vacías se ignoran.", false],
    ["   También puedes bajar cada sección por separado y subirlas de a una.", false],
    ["2. Borra la fila de ejemplo (va en gris) antes de subirla.", false],
