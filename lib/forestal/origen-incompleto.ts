...
import { ORIGEN_INVENTARIO_APERTURA, pieTablarDe } from "@/lib/forestal/lotes-aserrio";
import type { CorridaDisponible, LoteDeCapacidad } from "./capacidad-de-planta";

/**
 * Tres motivos, dos naturalezas. Los dos primeros son HUECOS: hay algo que
 * atar o completar. El tercero es un ESTADO (ADR-394): madera anterior al
 * libro, sin guía ni piezas que atar — se declara, no se corrige, y no se pinta
 * en rojo porque un rojo que nunca se puede apagar enseña a ignorar el rojo.
 */
export type MotivoOrigen = "sin_materia_prima" | "ingreso_sin_titulo" | "existencia_apertura";

/**
 * ¿Esta corrida es existencia de apertura? La ÚNICA función que lo decide
 * (ADR-394 §2): la declaró alguien con motivo, o entró por el importador del
 * libro SNIFFS con su marca. Nadie más vuelve a mirar el texto de la nota.
 */
export function esExistenciaDeApertura(
  c: Pick<CorridaDisponible, "aperturaDeclaradaAt" | "observations">,
): boolean {
  return (
    Boolean(c.aperturaDeclaradaAt) ||
    (c.observations ?? "").trim().startsWith(ORIGEN_INVENTARIO_APERTURA)
  );
}
...
export interface CorridaSinOrigen {
  ...
  pt: number | null;
  motivo: MotivoOrigen;
  /** Quién la declaró apertura y por qué, si fue a mano (ADR-394). */
  apertura?: { por: string | null; el: string | null; motivo: string | null; importada: boolean };
...
export interface ResumenOrigen {
  /** Los huecos corregibles (sin materia prima / ingreso sin título). */
  corridas: CorridaSinOrigen[];
  /** Las existencias de apertura: aparte, sin rojo, no certificables (ADR-394). */
  apertura: CorridaSinOrigen[];
  /** m³ (sólo unidad m³) que hoy no se pueden certificar por un HUECO corregible. */
  m3SinCertificar: number;
  /** m³ declarados existencia de apertura (tampoco certificables, pero no es un hueco). */
  m3Apertura: number;
  /** Del total disponible en m³, qué fracción es el hueco corregible (0..1). */
  fraccion: number;
...
export const MOTIVO_LABEL: Record<MotivoOrigen, string> = {
  ...
  existencia_apertura: "Existencia de apertura",
};
...
export function diagnosticar(
  ...
  /* Sólo una corrida SIN consumos puede ser apertura: con guía atada tiene origen. */
  const apertura = guias.length === 0 && esExistenciaDeApertura(c);
  return {
    id: c.id,
    fecha: c.fecha,
    lote: c.lote,
    ...
    apertura: apertura
      ? {
          por: c.aperturaDeclaradaPor ?? null,
          el: c.aperturaDeclaradaAt ?? null,
          motivo: c.aperturaDeclaradaMotivo ?? null,
          importada: !c.aperturaDeclaradaAt,
        }
      : undefined,
    guias,
...
export function resumenDeOrigen(
  ...
  const apertura = todas.filter((x) => x.motivo === "existencia_apertura");
  const filas = todas.filter((x) => x.motivo !== "existencia_apertura");
  ...
  const porMotivo: ResumenOrigen["porMotivo"] = {
    sin_materia_prima: { corridas: 0, m3: 0 },
    ingreso_sin_titulo: { corridas: 0, m3: 0 },
    existencia_apertura: { corridas: 0, m3: 0 },
  };
