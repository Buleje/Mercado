/**
 * Generador de QR sin dependencias externas (Reed-Solomon puro).
 *
 * Implementación mínima de QR versión 3-M (29×29, ~47 bytes / ~77 alfanuméricos)
 * suficiente para URLs cortas. Extraído de `app/api/products/[id]/qr/route.ts`
 * para reutilizarlo desde otros endpoints (junta, etc.) — single source.
 * Basado en el estándar ISO/IEC 18004:2015.
 */

// ── GF(256) — generador x^8 + x^4 + x^3 + x^2 + 1 (0x11d) ──────────────────────
const GF_EXP = new Uint8Array(512);
const GF_LOG = new Uint8Array(256);

(function buildGF() {
  let x = 1;
  for (let i = 0; i < 255; i++) {
    GF_EXP[i] = x;
    GF_LOG[x] = i;
    x = x << 1;
    if (x & 0x100) x ^= 0x11d;
  }
  for (let i = 255; i < 512; i++) GF_EXP[i] = GF_EXP[i - 255];
})();

/** Multiplica dos elementos de GF(256). */
function gfMul(a: number, b: number): number {
  if (a === 0 || b === 0) return 0;
  return GF_EXP[(GF_LOG[a] + GF_LOG[b]) % 255];
}

function rsEncode(data: number[], nEcc: number): number[] {
  let g = [1];
  for (let i = 0; i < nEcc; i++) {
    const factor = [1, GF_EXP[i]];
    const newG = Array.from({ length: g.length + factor.length - 1 }, () => 0);
    for (let j = 0; j < g.length; j++) {
      for (let k = 0; k < factor.length; k++) {
        newG[j + k] ^= gfMul(g[j], factor[k]);
      }
    }
    g = newG;
  }

  const msg = [...data, ...Array.from({ length: nEcc }, () => 0)];
  for (let i = 0; i < data.length; i++) {
    const c = msg[i];
    if (c !== 0) {
      for (let j = 1; j < g.length; j++) {
        msg[i + j] ^= gfMul(g[j], c);
      }
    }
  }
  return msg.slice(data.length);
}

// QR Version 3-M: 29×29, capacidad ~77 alfanuméricos / ~47 bytes.
export const QR_SIZE = 29;

function encodeBytes(text: string): { data: number[]; bitLen: number } {
  const bytes = [...Buffer.from(text, "utf8")];
  const bits: number[] = [];
  const push = (val: number, n: number) => {
    for (let i = n - 1; i >= 0; i--) bits.push((val >> i) & 1);
  };
  push(0b0100, 4); // byte mode
  push(bytes.length, 8);
  for (const b of bytes) push(b, 8);
  // Terminador
  for (let i = 0; i < 4 && bits.length < 26 * 8; i++) bits.push(0);
  // Pad a byte
  while (bits.length % 8 !== 0) bits.push(0);
  // Pad codewords
  const PAD = [0xec, 0x11];
  let pi = 0;
  while (bits.length < 26 * 8) {
    push(PAD[pi % 2], 8);
    pi++;
  }
  const data: number[] = [];
  for (let i = 0; i < bits.length; i += 8) {
    let b = 0;
    for (let j = 0; j < 8; j++) b = (b << 1) | (bits[i + j] ?? 0);
    data.push(b);
  }
  return { data, bitLen: bits.length };
}

/** Construye la matriz booleana del QR (true = módulo oscuro). */
export function makeQRMatrix(text: string): boolean[][] {
  const size = QR_SIZE;
  const mat: (boolean | null)[][] = Array.from({ length: size }, () =>
    Array.from({ length: size }, () => null),
  );

  const set = (r: number, c: number, v: boolean) => {
    mat[r][c] = v;
  };
  const isFree = (r: number, c: number) => mat[r]?.[c] === null;

  // Patrones de búsqueda (finder)
  const addFinder = (row: number, col: number) => {
    for (let r = -1; r <= 7; r++) {
      for (let c = -1; c <= 7; c++) {
        const inFinder = r >= 0 && r <= 6 && c >= 0 && c <= 6;
        if (row + r < 0 || row + r >= size || col + c < 0 || col + c >= size)
          continue;
        if (!inFinder) {
          set(row + r, col + c, false);
          continue;
        }
        const dark =
          r === 0 ||
          r === 6 ||
          c === 0 ||
          c === 6 ||
          (r >= 2 && r <= 4 && c >= 2 && c <= 4);
        set(row + r, col + c, dark);
      }
    }
  };
  addFinder(0, 0);
  addFinder(0, size - 7);
  addFinder(size - 7, 0);

  // Patrón de alineación (versión 3: uno en row=20,col=20)
  const addAlignment = (row: number, col: number) => {
    for (let r = -2; r <= 2; r++) {
      for (let c = -2; c <= 2; c++) {
        const dark =
          r === -2 || r === 2 || c === -2 || c === 2 || (r === 0 && c === 0);
        if (isFree(row + r, col + c)) set(row + r, col + c, dark);
      }
    }
  };
  addAlignment(20, 20);

  // Patrones de timing
  for (let i = 8; i < size - 8; i++) {
    if (isFree(6, i)) set(6, i, i % 2 === 0);
    if (isFree(i, 6)) set(i, 6, i % 2 === 0);
  }

  // Módulo oscuro (siempre)
  set(size - 8, 8, true);

  // Placeholders de format info (claros)
  for (let i = 0; i <= 8; i++) {
    if (isFree(8, i)) set(8, i, false);
    if (isFree(i, 8)) set(i, 8, false);
  }
  for (let i = size - 7; i < size; i++) {
    if (isFree(8, i)) set(8, i, false);
    if (isFree(i, 8)) set(i, 8, false);
  }

  // Codifica datos con RS
  const encoded = encodeBytes(text.slice(0, 47)); // máx bytes para v3-M
  const ecc = rsEncode(encoded.data, 26); // 26 EC codewords para v3-M
  const allBytes = [...encoded.data, ...ecc];

  const bits: boolean[] = [];
  for (const b of allBytes) {
    for (let i = 7; i >= 0; i--) bits.push(((b >> i) & 1) === 1);
  }

  // Coloca módulos de datos (zigzag)
  let bi = 0;
  for (let col = size - 1; col >= 1; col -= 2) {
    if (col === 6) col = 5; // salta columna de timing
    for (let row = 0; row < size; row++) {
      const actualRow =
        Math.floor((size - 1 - col) / 2) % 2 === 0 ? size - 1 - row : row;
      for (let dc = 0; dc < 2; dc++) {
        const c = col - dc;
        if (isFree(actualRow, c)) {
          set(actualRow, c, bits[bi] ?? false);
          bi++;
        }
      }
    }
  }

  // Máscara 0 (checkerboard: (r+c)%2==0)
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      if (mat[r][c] !== null && (r + c) % 2 === 0) {
        mat[r][c] = !(mat[r][c] as boolean);
      }
    }
  }

  // Format info (máscara 0, ECL M = 00) — patrón precomputado
  const FORMAT_V3_M_MASK0 = [
    true, false, true, false, true, false, false, false, false, false, true,
    false, false, true, false,
  ];
  const fmtPos = [
    [8, 0], [8, 1], [8, 2], [8, 3], [8, 4], [8, 5], [8, 7], [8, 8],
    [7, 8], [5, 8], [4, 8], [3, 8], [2, 8], [1, 8], [0, 8],
  ];
  const fmtPos2 = [
    [size - 1, 8], [size - 2, 8], [size - 3, 8], [size - 4, 8], [size - 5, 8],
    [size - 6, 8], [size - 7, 8], [8, size - 8], [8, size - 7], [8, size - 6],
    [8, size - 5], [8, size - 4], [8, size - 3], [8, size - 2], [8, size - 1],
  ];
  for (let i = 0; i < 15; i++) {
    const [r, c] = fmtPos[i];
    set(r, c, FORMAT_V3_M_MASK0[i]);
    const [r2, c2] = fmtPos2[i];
    set(r2, c2, FORMAT_V3_M_MASK0[i]);
  }

  return mat.map((row) => row.map((v) => v === true));
}

interface QrSvgOptions {
  cellSize?: number;
  /** Zona de silencio en celdas (mín. 4 para escaneo confiable). */
  quiet?: number;
  dark?: string;
  light?: string;
}

/** Renderiza la matriz como SVG plano (sin etiqueta). Server-safe, sin canvas. */
export function qrMatrixToSvg(
  matrix: boolean[][],
  opts: QrSvgOptions = {},
): string {
  const cellSize = opts.cellSize ?? 10;
  const quiet = opts.quiet ?? 4;
  const dark = opts.dark ?? "#111111";
  const light = opts.light ?? "#ffffff";
  const size = matrix.length;
  const px = (size + quiet * 2) * cellSize;

  const rects: string[] = [
    `<rect width="${px}" height="${px}" fill="${light}"/>`,
  ];
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      if (matrix[r][c]) {
        const x = (c + quiet) * cellSize;
        const y = (r + quiet) * cellSize;
        rects.push(
          `<rect x="${x}" y="${y}" width="${cellSize}" height="${cellSize}" fill="${dark}"/>`,
        );
      }
    }
  }

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${px}" height="${px}" viewBox="0 0 ${px} ${px}">
${rects.join("\n")}
</svg>`;
}
