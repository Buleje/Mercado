...
const PERMISSIONS: Partial<Record<Role, Partial<Record<Resource, Action[]>>>> = {
   ...
   * Audit 2026-05-17 05-P1-3: antes sin entrada en PERMISSIONS →
   * checkPermission devolvía false aunque require-admin lo pasara
   * por managementTier bypass. Inconsistencia entre runtime y declaración.
   * Ahora paridad explícita con admin (mismas keys, salvo "admin-users"
   * que requiere admin canónico).
   */
  owner: {
    orders: ["read", "write", "delete"],
    products: ["read", "write", "delete"],
  ...
  /**
   * MANAGER — Encargado. Sin delete sobre catálogo/clientes ni admin-users.
   */
  manager: {
    orders: ["read", "write", "delete"],
    products: ["read", "write"],
  ...
  /**
   * ANALISTA — Solo lectura para reportes y análisis.
   */
  analista: {
    orders: ["read"],
    products: ["read"],
    ...
    "activity-log": ["read"],
    analytics: ["read"],
    "demand-prediction": ["read"],
    "price-history": ["read"],
    store_analytics: ["read"],
    wholesale_orders: ["read"],
