// Aplica la migración 334 (lote de aserrío: trozas de una especie que van juntas
...
const sql = fs.readFileSync("prisma/manual-migrations/334-lote-aserrio.sql", "utf8");
...
const main = async () => {
  ...
  const cols = await c.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_name = 'ForestLoteAserrio' ORDER BY ordinal_position`,
  );
  console.log(`Columnas de ForestLoteAserrio (${cols.rows.length}):`, cols.rows.map((r) => r.column_name).join(", ") || "NINGUNA");
  const fk = await c.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_name = 'WoodEntryTroza' AND column_name = 'loteAserrioId'`,
  );
  console.log(`WoodEntryTroza.loteAserrioId: ${fk.rows.length ? "OK" : "FALTA"}`);
  await c.end();
  console.log("✅ Migración 334 aplicada. Después: npx prisma generate + reiniciar dev.");
};
