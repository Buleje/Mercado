// Aplica la migración 333 (ficha completa del beneficiario de adelantos:
...
const sql = fs.readFileSync("prisma/manual-migrations/333-cobranza-gestion.sql", "utf8");
...
const main = async () => {
  ...
  const cols = await c.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_name = 'AdelantoGestion' ORDER BY ordinal_position`,
  );
    console.log(`Columnas de AdelantoGestion (${cols.rows.length}):`, cols.rows.map((r) => r.column_name).join(", ") || "NINGUNA");
  await c.end();
  console.log("✅ Migración 333 aplicada. Después: npx prisma generate + reiniciar dev.");
};
