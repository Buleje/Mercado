/**
 * scripts/seed-mi-pollo.ts
 *
 * Crea un tenant nuevo "mi-pollo" con su menú completo: pollos a la brasa,
 * salchipapas, guarniciones, postres y bebidas.
 *
 * Uso:
 *   npx tsx -r dotenv/config scripts/seed-mi-pollo.ts
 *
 * Idempotente: si el tenant ya existe lo reutiliza y sincroniza los productos
 * (upsert por nombre + categoría).
 */
...
async function main() {
  ...
  const storeThemeJson = JSON.stringify(storeTheme);
  const productCategoriesJson = JSON.stringify(CATEGORIES);
