#!/usr/bin/env node
// Aplica prisma/migrations/adr-375-presupuesto-por-categoria.sql (tabla ExpenseBudget).
// CREATE INDEX CONCURRENTLY no puede correr dentro de transacción → ejecuta
// statement por statement con autocommit.
//
// Uso: node -r dotenv/config scripts/apply-375-migration.mjs dotenv_config_path=.env.local
...
if (!RAW_URL) {
  console.error("[adr-375] DIRECT_URL or DATABASE_URL missing — aborting");
  process.exit(1);
}
const url = new URL(RAW_URL);
const isPooler = url.hostname.includes("pooler.supabase.com");
if (isPooler && url.port === "6543") {
  console.log(`[adr-375] forzando session mode (5432) en pooler — CONCURRENTLY no funciona en transaction mode`);
  url.port = "5432";
}
const DIRECT_URL = url.toString();

const SQL_FILE = process.env.SQL_FILE || "prisma/migrations/adr-375-presupuesto-por-categoria.sql";
...
console.log(`[adr-375] ${statements.length} statements detected. Connecting to Supabase…`);
...
try {
  ...
  ipv4 = r.address;
  console.log(`[adr-375] resolved ${originalHost} → ${ipv4} (IPv4)`);
} catch (e) {
  console.error(`[adr-375] DNS IPv4 lookup failed for ${originalHost}: ${e?.message ?? e}`);
  console.error(`[adr-375] tip: el direct URL de Supabase solo tiene AAAA (IPv6).`);
  console.error(`[adr-375] usá DATABASE_URL (pooler con IPv4) — el script ya cambia 6543→5432 para soportar CONCURRENTLY.`);
  process.exit(1);
}
...
try {
  ...
  console.log(`[adr-375] connected in ${Date.now() - t0}ms`);
  ...
  console.log(`\n[adr-375] done in ${((Date.now() - t0) / 1000).toFixed(1)}s — ok=${ok} skipped=${skipped} failed=${failed}`);
  process.exit(failed > 0 ? 2 : 0);
} catch (e) {
  console.error(`[adr-375] connection error: ${e?.message ?? e}`);
  process.exit(1);
} finally {
