// Aplica la migración 326 (consumo por troza). Idempotente.
// Uso: DOTENV_CONFIG_PATH=.env.local node -r dotenv/config scripts/apply-326-migration.mjs
import fs from "node:fs";
import pg from "pg";

const sql = fs.readFileSync("prisma/manual-migrations/326-troza-consumida.sql", "utf8");
  ...
  const cols = await c.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_name = 'WoodEntryTroza'
       AND column_name IN ('consumidaEnId','fechaConsumo')
     ORDER BY column_name`,
  );
  console.log("Columnas:", cols.rows.map((r) => r.column_name).join(", ") || "NINGUNA");
  await c.end();
  console.log("✅ Migración 326 aplicada. Después: npx prisma generate + reiniciar dev.");
};
