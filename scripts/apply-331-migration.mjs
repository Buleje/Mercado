// Aplica la migración 331 (ficha completa del beneficiario de adelantos:
// identidad SUNAT/RENIEC, contacto, datos bancarios y baja lógica). Idempotente.
// Uso: DOTENV_CONFIG_PATH=.env.local node -r dotenv/config scripts/apply-331-migration.mjs
import fs from "node:fs";
import pg from "pg";

const sql = fs.readFileSync("prisma/manual-migrations/331-beneficiario-ubigeo.sql", "utf8");
  ...
  const cols = await c.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_name = 'AdelantoBeneficiario'
       AND column_name IN ('departamento','provincia','distrito')
     ORDER BY column_name`,
  );
    console.log(`Columnas (${cols.rows.length}/3):`, cols.rows.map((r) => r.column_name).join(", ") || "NINGUNA");
  await c.end();
  console.log("✅ Migración 331 aplicada. Después: npx prisma generate + reiniciar dev.");
};
