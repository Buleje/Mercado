...
|----|----------------|--------|-------------------|
| TD-022 | #9 Salida estructurada JSON | ❌ Platform-blocked | **Spike 2026-04-06 completado con llama-4-scout-17b-16e-instruct**: descarta Opción A. Groq bloquea `tools + response_format:json_object` con error HTTP 400 `"json mode cannot be combined with tool/function calling"` **en TODOS los modelos**, no solo llama-3.3. Es una limitación arquitectónica de Groq. **ADR 009 queda reducido a 2 opciones:** (B) prompt-based JSON enforcement con parsing + fallback (barato, 80% efectivo), (C) migrar endpoints críticos que necesiten JSON garantizado a Claude/OpenAI directo (caro, 100% efectivo). **Recomendación:** B para el ai-assistant principal, C solo si aparece un endpoint crítico (ej: parser de facturación SUNAT con schema estricto). **Bonus del spike:** llama-4-scout SÍ soporta `tools + streaming` (llama-3.3 no) — oportunidad de mejora UX independiente de #9: en `ai-assistant/route.ts` el first call podría ser streaming en vez de esperar a que termine para devolver respuesta. |
...
| TD-025 | #10 Human-in-the-Loop formal | ⚠️ Pre-staged | **Pre-stage 2026-04-06:** campo `requiresApproval?: boolean` agregado a `ToolDefinition` en `lib/agents/tool-definitions.ts` + 2 tools marcados como críticos (`notifications_send_order_update`, `notifications_send_promotion`). **Pendiente (1 sesión):** en el orchestrator interceptar tools con `requiresApproval=true`, emitir evento `pendingApproval`, y en `AICommandCenter.tsx` escuchar el evento y mostrar modal con botones Aprobar/Rechazar. Cuando tools futuros de compras/eliminación/descuentos se agreguen, marcarlos también. |
