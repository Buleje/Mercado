...
| TD-034 | e2e/ product-card selector | **Descubierto 2026-04-08** (sesión step Confirmar). Los 18 specs existentes en `e2e/` (+ los 3 nuevos de `checkout-confirmar-step.spec.ts`) esperan `[data-testid="product-card"]` sobre el home + marketplace, pero ese atributo **ya no existe en ningún componente** (`CategoryCatalog.tsx`, `marketplace/CatalogView.tsx`, `PersonalizedRecommendations.tsx`, etc.). Fix: agregar `data-testid="product-card"` a la tarjeta raíz del componente que renderiza productos en `/marketplace` + asegurarse que el botón de agregar matchee `/agregar/i`. Una sola línea por componente desbloquea 21 e2e de golpe. | 21 e2e bloqueados — sin red de seguridad browser para checkout/cart/navigation | 🔓 Abierto |
